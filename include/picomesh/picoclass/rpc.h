...
typedef struct picomesh_size_result (*rpc_skel_fn)(const void *body,
...
#define RPC_HDR_MAKE(op, id)                                                   \
  (((uint32_t)(op) << RPC_OP_SHIFT) | ((id) & RPC_ID_MASK))
...
int peer_channel_msgpack_call(struct peer_channel *s, const char *path,
                              struct yheaders *hdrs, const void *args,
...
typedef void (*rpc_async_oneway_fn)(void *ctx, enum rpc_op op, uint32_t id,
                                    const void *body, size_t body_len);
void peer_channel_set_async_oneway(struct peer_channel *s,
...
typedef int (*rpc_msgpack_io_fn)(void *ctx, const uint8_t *env, size_t env_len,
                                 uint8_t **resp, size_t *resp_len, char *err,
                                 size_t err_cap);
void peer_channel_set_msgpack_async(struct peer_channel *s, void *ctx,
                                    rpc_msgpack_io_fn io,
                                    void (*destroy)(void *ctx));

size_t rpc_call(struct peer_channel *s, enum rpc_op op, uint32_t id,
...
void peer_channel_set_remote_id(struct peer_channel *s, method_slot local_slot,
                                uint32_t remote_id);

struct picomesh_int_result peer_channel_translate_class(struct peer_channel *s,
                                                        const char *class_name);
struct picomesh_uint32_result
peer_channel_ensure_remote_id(struct peer_channel *s, method_slot local_slot);
