...
/* ---- methods ---- */
struct picomesh_json_result
relational_storage_db_exec(struct ctx *ctx, struct object *obj,
                           struct yheaders *hdrs, const char *db_name,
                           uint32_t shard_key, const char *sql,
                           const char *args_json);
struct picomesh_json_result
relational_storage_db_query(struct ctx *ctx, struct object *obj,
                            struct yheaders *hdrs, const char *db_name,
                            uint32_t shard_key, const char *sql,
                            const char *args_json);
struct picomesh_int_result
relational_storage_db_shard_count(struct ctx *ctx, struct object *obj,
                                  struct yheaders *hdrs, const char *db_name);
