                          ...
                          void *_resp, size_t _resp_max)
{
    ...
    _off += sizeof(_v2);
    struct yaafc_uint32_result _r = token_issuer_store_login(&_local, _obj, _v1, _v2);
                          ...
                          void *_resp, size_t _resp_max)
{
    ...
    memcpy(&_v1, (const uint8_t *)_body + _off, sizeof(_v1));
    _off += sizeof(_v1);
    struct yaafc_uint32_result _r = token_issuer_store_validate(&_local, _obj, _v1);
    if (_resp_max < 1) return 0;
    if (YAAFC_IS_ERR(_r)) {
                          ...
                          void *_resp, size_t _resp_max)
{
    ...
    memcpy(&_v1, (const uint8_t *)_body + _off, sizeof(_v1));
    _off += sizeof(_v1);
    struct yaafc_uint32_result _r = token_issuer_store_refresh(&_local, _obj, _v1);
    if (_resp_max < 1) return 0;
    if (YAAFC_IS_ERR(_r)) {
                          ...
                          void *_resp, size_t _resp_max)
{
    ...
    memcpy(&_v1, (const uint8_t *)_body + _off, sizeof(_v1));
    _off += sizeof(_v1);
    struct yaafc_int_result _r = token_issuer_store_revoke(&_local, _obj, _v1);
    if (_resp_max < 1) return 0;
    if (YAAFC_IS_ERR(_r)) {
                          ...
                          void *_resp, size_t _resp_max)
{
    ...
    struct yaafc_size_result _r = token_issuer_store_count_active(&_local, _obj);
    if (_resp_max < 1) return 0;
    if (YAAFC_IS_ERR(_r)) {
...
struct object_ptr_result token_issuer_store_create(struct ctx *ctx)
{
    ...
    rpc_session_translate_class(ctx->session, "token_issuer_store");

    uint64_t _h = 0;
    const char *_name = "token_issuer_store";
    if (rpc_call(ctx->session, RPC_OP_CREATE, 0, _name, strlen(_name),
                 &_h, sizeof(_h)) != sizeof(_h) || !_h)
        return YAAFC_ERR(object_ptr, "token_issuer_store_create: remote create failed");

    void *_mem = calloc(1, sizeof(struct object) + sizeof(uint64_t));
    if (!_mem)
        return YAAFC_ERR(object_ptr, "token_issuer_store_create: calloc(proxy) failed");
