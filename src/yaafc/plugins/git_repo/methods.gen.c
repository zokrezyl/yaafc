...
struct yaafc_string_result git_repo_store_list_for_owner(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t owner_id)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (YAAFC_IS_ERR(_sr))
            return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: NULL object");

    struct ctx *_s = ctx;
    if (_s && _s->peer) {
        uint32_t _rid = peer_channel_ensure_remote_id(_s->peer, _slot);
        if (_rid == RPC_REMOTE_ID_UNRESOLVED)
            return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: remote id unresolved");
        uint8_t _a[16384];
        size_t _off = 0;
        /* Headers section: the FRAMEWORK serializes the request-header
         * bag (uid, sid, trace_id, or anything a caller injected) ahead
         ...
         * contents — it just lets the framework (de)serialize the bag. */
        {
            size_t _hn = yheaders_serialize(hdrs, _a, sizeof(_a));
            if (_hn == 0)
                return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: header serialize overflow");
            _off = _hn;
        }
        {
            uint64_t _h = *(uint64_t *)((char *)obj + sizeof(*obj));
            if (_off + 8 > sizeof(_a))
                return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: pack overflow");
            memcpy(_a + _off, &_h, 8); _off += 8;
        }
        if (_off + sizeof(owner_id) > sizeof(_a))
            return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: pack overflow");
        ...
        if (_wn < 1) return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: short RPC response");
        if (_wbuf[0] != 0) {
            uint32_t _msg_len = 0;
            if (_wn >= 5) memcpy(&_msg_len, _wbuf + 1, 4);
            char _msg[260];
            ...
            if (_wn >= 5 + _copy) memcpy(_msg, _wbuf + 5, _copy);
            _msg[_copy] = 0;
            return YAAFC_ERR(yaafc_string, _msg[0] ? strdup(_msg) : "git_repo_store_list_for_owner: remote error (no msg)");
        }
        if (_wn < 5) return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: truncated string response");
        uint32_t _slen;
        memcpy(&_slen, _wbuf + 1, 4);
        if (_wn < (size_t)5 + _slen) return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: truncated string payload");
        char *_sv = malloc((size_t)_slen + 1);
        if (!_sv) return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: out of memory");
        if (_slen) memcpy(_sv, _wbuf + 5, _slen);
        _sv[_slen] = 0;
        return YAAFC_OK(yaafc_string, _sv);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return YAAFC_ERR(yaafc_string, "git_repo_store_list_for_owner: no impl on this class");
