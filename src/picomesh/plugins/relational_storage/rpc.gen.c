...
static size_t relational_storage_db_exec_skel(const void *_body, size_t _body_len,
...
static size_t relational_storage_db_query_skel(const void *_body, size_t _body_len,
                          void *_resp, size_t _resp_max)
{
    size_t _off = 0;
    struct ctx _local = {0};
...
static int relational_storage_db_query_jinvoke(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                          const struct yjson_value *args,
                          struct yjson_writer *result, char *err, size_t err_cap)
{
    uint32_t arg0 = (uint32_t)yjson_as_int(yjson_array_at(args, 0), 0);
...
static int relational_storage_db_exec_minvoke(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
...
static int relational_storage_db_query_minvoke(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                          cmp_ctx_t *_mr, uint32_t _argc, cmp_ctx_t *_mw,
                          char *_err, size_t _err_cap)
{
    (void)_mr;
...
void picomesh_plugin_relational_storage_register(void)
{
    ...
    if (PICOMESH_IS_ERR(_ar)) {
        picomesh_error_print(stderr, "picomesh_plugin_relational_storage_register", _ar.error);
