...
struct picomesh_string_result runner_agent_runner_agent_exchange(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, const char * token)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        struct method_slot_result _sr =
            method_slot_get("runner_agent", (method_id_t)runner_agent_runner_agent_exchange);
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_string, "runner_agent_runner_agent_exchange: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_string, "runner_agent_runner_agent_exchange: NULL object");
    ...
    if (_s && _s->peer) {
        if (peer_channel_is_msgpack(_s->peer)) {
            ...
            if (!_margs || !_mresp) {
                free(_margs); free(_mresp);
                return PICOMESH_ERR(picomesh_string, "runner_agent_runner_agent_exchange: out of memory");
            }
            struct picomesh_msgpack_buffer _mab;
            cmp_ctx_t _maw;
            picomesh_msgpack_writer_init(&_maw, &_mab, _margs, 16384);
            ...
            uint32_t _mssz = 0;
            if (!cmp_read_str_size(&_mrr, &_mssz)) {
                _mret = PICOMESH_ERR(picomesh_string, "runner_agent_runner_agent_exchange: bad msgpack string result");
            } else if (_mrb.offset + _mssz > _mrlen) {
                _mret = PICOMESH_ERR(picomesh_string, "runner_agent_runner_agent_exchange: truncated msgpack string");
            } else {
                char *_msv = malloc((size_t)_mssz + 1);
                if (!_msv) _mret = PICOMESH_ERR(picomesh_string, "runner_agent_runner_agent_exchange: out of memory");
                else {
                    if (_mssz) memcpy(_msv, _mresp + _mrb.offset, _mssz);
                    _msv[_mssz] = 0;
         ...
         * peer's parent. */
        struct ytelemetry_span _tsp;
        ytelemetry_client_span_begin(&_tsp, hdrs, "rpc.runner_agent_runner_agent_exchange");
        /* Headers section: the FRAMEWORK serializes the request-header
         * bag (uid, trace context, or anything a caller injected) ahead
         * of the packed business args. The skel parses it straight back
         * into the `hdrs` argument. ytelemetry_client_serialize_headers swaps in
    ...
    _rpc_done:
        ...
        return ((runner_agent_runner_agent_exchange_fn)fn)(ctx, obj, hdrs, token);
    }
}
