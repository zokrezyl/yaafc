 ...
 *   ARGV_BOOL       — set the flag, no value consumed.
 *   ARGV_VALUE      — consume the next argv token, store last value.
 *   ARGV_KEY_VALUE  — consume the next argv token, append to a list.
 *
...
struct argv_value_slot {
    const struct argv_option_def *def;
    int present;         /* set at least once */
    int bool_val;        /* ARGV_BOOL */
    char *string_val;    /* ARGV_VALUE: last value wins */
    char **kv_list;      /* ARGV_KEY_VALUE+multiple: accumulated values */
    size_t kv_count;
    size_t kv_cap;
};

struct argv_chain {
    const struct argv_option_def *defs;
    size_t def_count;
    struct argv_value_slot *slots; /* def_count entries */
...
static const char *def_dest(const struct argv_option_def *def, char *scratch, size_t scratch_len)
{
...
static struct argv_value_slot *slot_by_dest(struct argv_chain *chain, const char *dest)
{
...
static const struct argv_value_slot *slot_by_dest_const(const struct argv_chain *chain, const char *dest)
{
    return slot_by_dest((struct argv_chain *)chain, dest);
}

static int kv_push(struct argv_value_slot *slot, const char *kv)
{
...
static struct argv_value_slot *match(struct argv_chain *chain, const char *tok)
{
    if (!tok) return NULL;
    for (size_t i = 0; i < chain->def_count; ++i) {
        const struct argv_option_def *def = &chain->defs[i];
...
{
    struct argv_chain *chain = calloc(1, sizeof(*chain));
    if (!chain) return PICOMESH_ERR(argv_chain_ptr, "argv_parse: calloc failed");
    chain->defs = defs;
    chain->def_count = def_count;
    chain->slots = calloc(def_count ? def_count : 1, sizeof(*chain->slots));
    if (def_count && !chain->slots) {
        free(chain);
        return PICOMESH_ERR(argv_chain_ptr, "argv_parse: slots alloc failed");
    }
    ...
    while (i < argc) {
        ...
        if (!options_ended && tok[0] == '-' && tok[1] != '\0') {
            struct argv_value_slot *slot = match(chain, tok);
            if (!slot) {
            ...
            case ARGV_VALUE:
                if (i + 1 >= argc) {
                    argv_chain_destroy(chain);
                    return PICOMESH_ERR(argv_chain_ptr, "argv_parse: missing value for option");
                }
                slot->present = 1;
                ...
                if (!slot->string_val) {
                    argv_chain_destroy(chain);
                    return PICOMESH_ERR(argv_chain_ptr, "argv_parse: strdup failed");
                }
                i += 2;
                break;
            case ARGV_KEY_VALUE:
                if (i + 1 >= argc) {
                    argv_chain_destroy(chain);
                    return PICOMESH_ERR(argv_chain_ptr, "argv_parse: missing key=value for option");
                }
                slot->present = 1;
                if (kv_push(slot, argv[i + 1]) < 0) {
                    argv_chain_destroy(chain);
                    return PICOMESH_ERR(argv_chain_ptr, "argv_parse: kv push failed");
                }
                i += 2;
                break;
            }
    ...
    return PICOMESH_OK(argv_chain_ptr, chain);
}

void argv_chain_destroy(struct argv_chain *chain)
{
    if (!chain) return;
    for (size_t i = 0; i < chain->def_count; ++i) {
...
const char *argv_get_string(const struct argv_chain *chain, const char *dest, const char *fallback)
{
    const struct argv_value_slot *slot = slot_by_dest_const(chain, dest);
    if (slot && slot->present && slot->string_val) return slot->string_val;
    return fallback;
}

int argv_get_bool(const struct argv_chain *chain, const char *dest, int fallback)
{
    const struct argv_value_slot *slot = slot_by_dest_const(chain, dest);
...
int64_t argv_get_int(const struct argv_chain *chain, const char *dest, int64_t fallback)
{
    const struct argv_value_slot *slot = slot_by_dest_const(chain, dest);
...
size_t argv_get_kv_list(const struct argv_chain *chain, const char *dest,
...
const char *argv_subcommand(const struct argv_chain *chain)
{
    return chain ? chain->subcommand : NULL;
}

int argv_sub_argc(const struct argv_chain *chain)
{
    return chain ? chain->sub_argc : 0;
}

char *const *argv_sub_argv(const struct argv_chain *chain)
{
    return chain ? chain->sub_argv : NULL;
}

/* Build the left "  --long, -short <ARG>" column for one option. */
static int argv_opt_synopsis(const struct argv_option_def *def, char *buf, size_t cap)
{
    const char *arg = "";
    switch (def->kind) {
    case ARGV_VALUE:     arg = " VALUE"; break;
    case ARGV_KEY_VALUE: arg = " K=V";   break;
    case ARGV_BOOL:      arg = "";       break;
    }
...
void argv_print_options(const struct argv_option_def *defs, size_t def_count, FILE *out)
{
    ...
    for (size_t i = 0; i < def_count; ++i) {
        char buf[96];
        int synopsis_len = argv_opt_synopsis(&defs[i], buf, sizeof(buf));
        if (synopsis_len > width) width = synopsis_len;
    }
    for (size_t i = 0; i < def_count; ++i) {
        char buf[96];
        argv_opt_synopsis(&defs[i], buf, sizeof(buf));
