...
static int ytel_enabled(void)
{
    ...
    if (cached < 0) {
        ...
        if (env && (!strcasecmp(env, "off") || !strcasecmp(env, "false") ||
            ...
            if (cfg) {
                struct yconfig_node_ptr_result config_res = yconfig_get(cfg, "telemetry.enabled");
                if (PICOMESH_IS_OK(config_res) && config_res.value)
                    enabled = yconfig_node_as_bool(config_res.value, 1) ? 1 : 0;
...
static const char *ytel_service_name(void)
{
    ...
    if (!done) {
        struct picomesh_engine *engine = picomesh_active_engine();
        const char *name_arg = engine ? yargv_get_string(picomesh_engine_cli(engine), "name", NULL)
                                      : NULL;
...
static const struct ytel_batch_cfg *ytel_batch_config(void)
{
    ...
    if (!loaded) {
        ...
        struct picomesh_engine *engine = picomesh_active_engine();
        const struct yconfig *config = engine ? picomesh_engine_config(engine) : NULL;
        if (config) {
            struct yconfig_node_ptr_result node_res;
            node_res = yconfig_get(config, "telemetry.batch_max_spans");
            if (PICOMESH_IS_OK(node_res) && node_res.value) { int64_t value = yconfig_node_as_int(node_res.value, 0); if (value > 0) cfg.max_spans = (int)value; }
            else if (PICOMESH_IS_ERR(node_res)) picomesh_error_destroy(node_res.error);
            node_res = yconfig_get(config, "telemetry.batch_max_bytes");
            if (PICOMESH_IS_OK(node_res) && node_res.value) { int64_t value = yconfig_node_as_int(node_res.value, 0); if (value > 0) cfg.max_bytes = (size_t)value; }
            else if (PICOMESH_IS_ERR(node_res)) picomesh_error_destroy(node_res.error);
            node_res = yconfig_get(config, "telemetry.batch_flush_ms");
            if (PICOMESH_IS_OK(node_res) && node_res.value) { int64_t value = yconfig_node_as_int(node_res.value, 0); if (value > 0) cfg.flush_ns = (uint64_t)value * 1000000ull; }
            else if (PICOMESH_IS_ERR(node_res)) picomesh_error_destroy(node_res.error);
        }
        loaded = 1;
    }
...
static void ytel_flush_batch(void)
{
    ...
    if (ingest_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(slot_res)) { picomesh_error_destroy(slot_res.error); batch->count = 0; batch->len = 0; return; }
        ingest_slot = slot_res.value;
    }
    struct object_ptr_result object_res = object_create_in_ctx(&ctx, "trace_collector_trace_collector");
    if (PICOMESH_IS_ERR(object_res)) { picomesh_error_destroy(object_res.error); batch->count = 0; batch->len = 0; return; }
    struct object *object = object_res.value;
    uint32_t remote_id = peer_channel_ensure_remote_id(ctx.peer, ingest_slot);
