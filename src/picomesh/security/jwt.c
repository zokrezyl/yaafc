...
#include <picomesh/security/base64.h>
...
{
    return picomesh_platform_time_wall_ms() / 1000;
}
                                ...
                                const char *groups_csv, int64_t issued_at, int64_t expires_at)
{
    ...
    json_writer_key(writer, "iss");      json_writer_string(writer, issuer ? issuer : "picomesh");
    json_writer_key(writer, "sub");      json_writer_int(writer, (int64_t)sub);
    json_writer_key(writer, "username"); json_writer_string(writer, username ? username : "");
    json_writer_key(writer, "groups");   json_writer_begin_array(writer);
    if (groups_csv && *groups_csv) {
        ...
        while (*cursor) {
            ...
            if (span > 0) {
                ...
                group[span] = 0;
                json_writer_string(writer, group);
            }
            if (!comma) break;
            cursor = comma + 1;
        }
    ...
    json_writer_key(writer, "iat"); json_writer_int(writer, issued_at);
    json_writer_key(writer, "exp"); json_writer_int(writer, expires_at);
    ...
    json_writer_key(writer, "jti"); json_writer_string(writer, jti);
...
struct picomesh_string_result picomesh_jwt_verify(const char *jwt, const char *secret, int64_t now)
{
    ...
    struct json_doc *header_doc = json_parse(header_text, strlen(header_text));
    if (!header_doc) return PICOMESH_ERR(picomesh_string, "jwt_verify: header not JSON");
    const char *alg = json_as_string(json_object_get(json_doc_root(header_doc), "alg"), "");
    int alg_ok = strcmp(alg, "HS256") == 0;
    json_doc_free(header_doc);
    ...
    /* Check expiry. */
    struct json_doc *payload_doc = json_parse(payload_json, payload_len);
    ...
    int64_t exp = json_as_int(json_object_get(json_doc_root(payload_doc), "exp"), 0);
    json_doc_free(payload_doc);
                                                      ...
                                                      struct picomesh_authctx *out)
{
    ...
    struct json_doc *doc = json_parse(verified.value, strlen(verified.value));
    ...
    out->uid = (uint32_t)json_as_int(json_object_get(root, "sub"), 0);
    const char *username = json_as_string(json_object_get(root, "username"), "");
    snprintf(out->username, sizeof(out->username), "%s", username);

    /* Flatten the groups array to a CSV the role helpers consume. */
    const struct json_value *groups = json_object_get(root, "groups");
    size_t group_count = groups ? json_array_size(groups) : 0;
    size_t pos = 0;
    for (size_t i = 0; i < group_count; ++i) {
        const char *group = json_as_string(json_array_at(groups, i), NULL);
