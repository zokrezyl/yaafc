...
/* Deterministic password hash so we can keep typing a real password while the
 * credential plugin still indexes on int64 (its wire shape today). FNV-1a, no
 * salt — fine for a demo. (UIDs are no longer hash-derived: they are assigned,
 * never-reused ids allocated by the accounts service and resolved per username
 * via accounts.uid_for_username — issue #29.) */
...
static struct picomesh_int_result is_site_admin(uint32_t uid);
static struct picomesh_void_result landing_url(uint32_t uid, const char *uname,
                                               char *out, size_t cap);

static struct picomesh_void_result render_head(struct buf *buf,
                                               const char *title, uint32_t uid,
                                               const char *uname) {
  buf_printf(
      buf,
  ...
  buf_puts(
      ...
      "  border: 1px solid var(--border);\n"
      "}\n"
      ".badge.open, .badge.running, .badge.queued { background: #ddf4ff; "
      "color: #0969da; border-color: #b6e3ff; }\n"
      ".badge.closed, .badge.succeeded { background: #dafbe1; color: "
      "var(--success); border-color: #b5e6c6; }\n"
      ".badge.failed, .badge.timeout, .badge.cancelled { background: #ffebe9; "
      "color: var(--danger); border-color: #ffcecb; }\n"
      ".badge.priority-low      { background: #fff; color: var(--muted); }\n"
      ".badge.priority-medium   { background: #fff8c5; color: var(--warn); "
      "border-color: #ead97c; }\n"
      ".badge.priority-high     { background: #ffebe9; color: var(--danger); "
      "border-color: #ffcecb; }\n"
      ...
      "  border-color: #24292f;\n"
      "}\n"
      ".oauth-btn:hover { background: #1b1f24; color: #fff; text-decoration: "
      "none; }\n"
      "\n"
      "/* runners */\n"
      ".chip {\n"
  ...
  if (uid) {
    ...
    struct picomesh_int_result admin_res = is_site_admin(uid);
    if (PICOMESH_IS_ERR(admin_res)) {
      picomesh_error_print(stderr, "is_site_admin", admin_res.error);
      picomesh_error_destroy(admin_res.error);
    } else if (admin_res.value) {
...
static struct picomesh_void_result send_html(struct loop_stream *stream,
  ...
  if (written <= 0)
    return PICOMESH_ERR(picomesh_void, "send_html: header format failed");
  struct picomesh_size_result header_write =
...
static struct picomesh_void_result send_redirect(struct loop_stream *stream,
                                                 const char *where,
                                                 int keep_alive,
  ...
  if (written <= 0)
    return PICOMESH_ERR(picomesh_void, "send_redirect: header format failed");
  struct picomesh_size_result write_res =
      loop_write(stream, header, (size_t)written);
  ...
  while (cursor < end) {
    ...
    if (line_end - cursor > 7 && strncasecmp(cursor, "Cookie:", 7) == 0) {
      ...
      while (pair < line_end) {
        ...
        if (!eq)
          break;
        if ((size_t)(eq - pair) == name_len &&
            memcmp(pair, name, name_len) == 0) {
          const char *value_start = eq + 1;
...
static struct picomesh_int_result extract_session_token(const char *headers_raw,
                                                        size_t headers_raw_len,
                                                        char *out, size_t cap);
static struct picomesh_uint32_result uid_for_token(const char *token);
static struct picomesh_void_result
resolve_authctx(const char *headers_raw, size_t headers_raw_len,
...
#define SVC_OPEN(VAR, SVC, CREATE_FN)                                          \
  ...
  do {                                                                         \
    ...
    if (PICOMESH_IS_ERR(_o)) {                                                 \
      picomesh_error_destroy(_o.error);                                        \
      break;                                                                   \
    }                                                                          \
    VAR.obj = _o.value;                                                        \
    VAR.ok = 1;                                                                \
...
static inline void svc_close_impl(struct svc_ctx *svc) { (void)svc; }
...
static struct picomesh_void_result render_auth_form(struct loop_stream *stream,
                                                    int is_register,
                                                    const char *error,
                                                    int keep_alive) {
  struct buf body_buf;
  ...
  struct picomesh_void_result send_res =
      send_html(stream, 200, body_buf.data, body_buf.len, keep_alive, NULL);
  buf_free(&body_buf);
  return send_res;
}

static struct picomesh_void_result
render_login(struct loop_stream *stream, const char *error, int keep_alive) {
  return render_auth_form(stream, /*is_register=*/0, error, keep_alive);
}
static struct picomesh_void_result
render_register(struct loop_stream *stream, const char *error, int keep_alive) {
...
static struct picomesh_void_result build_session_cookies(char *out, size_t cap,
  ...
  if (written <= 0 || (size_t)written >= cap)
    return PICOMESH_ERR(picomesh_void,
                        "build_session_cookies: cookie header overflow");
  return PICOMESH_OK_VOID();
}
 ...
 * tok_out. */
static struct picomesh_int_result
auth_and_start_session(uint32_t uid, const char *username, int64_t pw,
                       char *tok_out, size_t tok_cap, const char **err) {
  ...
  if (!token_issuer_svc.ok) {
    *err = "token_issuer unreachable";
    return PICOMESH_OK(picomesh_int, 0);
  }
  struct picomesh_json_result login = token_issuer_token_issuer_login(
      &token_issuer_svc.c, token_issuer_svc.obj, NULL, "password", uid,
      username ? username : "", pw);
  SVC_CLOSE(token_issuer_svc);
  if (PICOMESH_IS_ERR(login)) {
    picomesh_error_print(stderr, "auth_and_start_session: token_issuer login",
                         login.error);
  ...
  struct json_doc *doc = json_parse(login.value, strlen(login.value));
  if (doc) {
    const struct json_value *root = json_doc_root(doc);
    const char *access_jwt_str =
        json_as_string(json_object_get(root, "access_jwt"), NULL);
    const char *refresh_token_str =
        json_as_string(json_object_get(root, "refresh_token"), NULL);
    if (access_jwt_str)
      snprintf(access_jwt, sizeof(access_jwt), "%s", access_jwt_str);
  ...
  free(login.value);
  if (!access_jwt[0]) {
    *err = "login produced no token";
    return PICOMESH_OK(picomesh_int, 0);
  }

  SVC_OPEN(session_svc, "session", session_session_create);
  if (!session_svc.ok) {
    *err = "session unreachable";
    return PICOMESH_OK(picomesh_int, 0);
  }
  struct picomesh_string_result start_res = session_session_start(
      &session_svc.c, session_svc.obj, NULL, uid, access_jwt, refresh_token);
  SVC_CLOSE(session_svc);
  if (PICOMESH_IS_ERR(start_res)) {
    picomesh_error_print(stderr, "auth_and_start_session: session start",
                         start_res.error);
...
static struct picomesh_void_result register_release_claim(uint32_t uid,
                                                          const char *uname);
static struct picomesh_void_ptr_result internal_caps(uint32_t uid);
static struct picomesh_void_result send_json_error(struct loop_stream *stream,
                                                   int status,
                                                   const char *message,
                                                   int keep_alive);
static struct picomesh_int_result header_value(const char *raw, size_t raw_len,
                                               const char *name, char *out,
                                               size_t out_cap);

static struct picomesh_void_result oauth_start_session(uint32_t uid,
                                                       const char *username,
                                                       char *tok_out,
                                                       size_t tok_cap) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_ERR(picomesh_void, "oauth_start_session: no active engine");
  SVC_OPEN(accounts_svc, "accounts", accounts_accounts_create);
  if (!accounts_svc.ok)
    return PICOMESH_ERR(picomesh_void,
                        "oauth_start_session: accounts service unreachable");
  struct picomesh_string_result groups =
      accounts_accounts_groups(&accounts_svc.c, accounts_svc.obj, NULL, uid);
  SVC_CLOSE(accounts_svc);
  PICOMESH_RETURN_IF_ERR(picomesh_void, groups,
                         "oauth_start_session: load account groups");

  struct picomesh_void_ptr_result sys_caps_res = internal_caps(uid);
  PICOMESH_RETURN_IF_ERR(picomesh_void, sys_caps_res,
                         "internal_caps: capability mint failed");
  struct yheaders *sys_caps = sys_caps_res.value;
  if (!sys_caps) {
    free(groups.value);
    return PICOMESH_ERR(picomesh_void,
                        "oauth_start_session: internal capability unavailable");
  }
  SVC_OPEN(token_issuer_svc, "token_issuer", token_issuer_token_issuer_create);
  if (!token_issuer_svc.ok) {
    ...
    free(groups.value);
    return PICOMESH_ERR(picomesh_void,
                        "oauth_start_session: token_issuer unreachable");
  }
  struct picomesh_string_result access = token_issuer_token_issuer_mint(
      &token_issuer_svc.c, token_issuer_svc.obj, sys_caps, uid,
      username ? username : "", groups.value ? groups.value : "", 0);
  SVC_CLOSE(token_issuer_svc);
  yheaders_free(sys_caps);
  free(groups.value);
  if (PICOMESH_IS_ERR(access))
    return PICOMESH_ERR(picomesh_void, "oauth_start_session: token mint failed",
                        access);
  ...
  if (!session_svc.ok) {
    free(access.value);
    return PICOMESH_ERR(picomesh_void,
                        "oauth_start_session: session unreachable");
  }
  struct picomesh_string_result start_res =
      session_session_start(&session_svc.c, session_svc.obj, NULL, uid,
                            access.value ? access.value : "", "");
  SVC_CLOSE(session_svc);
  free(access.value);
  if (PICOMESH_IS_ERR(start_res))
    return PICOMESH_ERR(
        picomesh_void, "oauth_start_session: session create failed", start_res);
  snprintf(tok_out, tok_cap, "%s", start_res.value ? start_res.value : "");
  free(start_res.value);
  if (!tok_out[0])
    return PICOMESH_ERR(
        picomesh_void, "oauth_start_session: session create produced no token");
  return PICOMESH_OK_VOID();
}

static struct picomesh_void_result ensure_oauth_account(uint32_t uid,
                                                        const char *uname) {
  ...
  if (!acc_chk.ok)
    return PICOMESH_ERR(picomesh_void,
                        "ensure_oauth_account: accounts service unreachable");
  struct picomesh_int_result exists_res =
      accounts_accounts_exists(&acc_chk.c, acc_chk.obj, NULL, uid);
  SVC_CLOSE(acc_chk);
  if (PICOMESH_IS_ERR(exists_res))
    return PICOMESH_ERR(picomesh_void,
                        "ensure_oauth_account: account lookup failed",
                        exists_res);
  if (exists_res.value)
    return PICOMESH_OK_VOID();
  ...
  if (!acc_claim.ok)
    return PICOMESH_ERR(picomesh_void,
                        "ensure_oauth_account: accounts service unreachable");
  struct picomesh_int_result claim_res = accounts_accounts_claim_username(
      &acc_claim.c, acc_claim.obj, NULL, uid, uname);
  SVC_CLOSE(acc_claim);
  if (PICOMESH_IS_ERR(claim_res))
    return PICOMESH_ERR(picomesh_void,
                        "ensure_oauth_account: username claim failed",
                        claim_res);
  if (claim_res.value != 1)
    return PICOMESH_ERR(
        picomesh_void,
        "ensure_oauth_account: GitHub login name is already taken");
  ...
  if (!sys_caps) {
    ...
    return PICOMESH_ERR(
        picomesh_void,
        "ensure_oauth_account: registration temporarily unavailable");
  }
  ...
  struct picomesh_string_result personal_ns = accounts_accounts_ns_create(
      &acc_ns.c, acc_ns.obj, sys_caps, uid, "user", uname, "");
  SVC_CLOSE(acc_ns);
  yheaders_free(sys_caps);
  if (PICOMESH_IS_ERR(personal_ns)) {
    register_release_claim(uid, uname);
    return PICOMESH_ERR(
        picomesh_void, "ensure_oauth_account: personal namespace create failed",
        personal_ns);
  }
  ...
  struct picomesh_size_result count_res =
      accounts_accounts_count(&acc_boot.c, acc_boot.obj, NULL);
  if (PICOMESH_IS_OK(count_res))
    first_user = (count_res.value == 0);
  else
    picomesh_error_destroy(count_res.error);
  if (first_user) {
    struct picomesh_void_ptr_result site_caps_res = internal_caps(uid);
    PICOMESH_RETURN_IF_ERR(picomesh_void, site_caps_res,
                           "internal_caps: capability mint failed");
    struct yheaders *site_caps = site_caps_res.value;
    if (!site_caps) {
      SVC_CLOSE(acc_boot);
      register_release_claim(uid, uname);
      return PICOMESH_ERR(
    ...
    struct picomesh_string_result site = accounts_accounts_ns_create(
        &acc_boot.c, acc_boot.obj, site_caps, uid, "group", "site", "");
    ...
    if (PICOMESH_IS_OK(site)) {
      ...
      if (PICOMESH_IS_ERR(present))
        picomesh_error_destroy(present.error);
      if (!site_present) {
        SVC_CLOSE(acc_boot);
        register_release_claim(uid, uname);
        return PICOMESH_ERR(picomesh_void,
                            "ensure_oauth_account: registration temporarily "
                            "unavailable (site bootstrap)");
      }
    }
  }
  SVC_CLOSE(acc_boot);
  ...
  struct picomesh_int_result register_res =
      accounts_accounts_register(&acc_reg.c, acc_reg.obj, NULL, uid, uname);
  SVC_CLOSE(acc_reg);
  ...
  if (!registered) {
    if (created_site) {
      int rollback_ok = 0;
      struct picomesh_void_ptr_result del_caps_res = internal_caps(uid);
      PICOMESH_RETURN_IF_ERR(picomesh_void, del_caps_res,
                             "internal_caps: capability mint failed");
      struct yheaders *del_caps = del_caps_res.value;
      if (del_caps) {
        SVC_OPEN(acc_rb, "accounts", accounts_accounts_create);
        if (acc_rb.ok) {
          struct picomesh_int_result delete_res = accounts_accounts_ns_delete(
              &acc_rb.c, acc_rb.obj, del_caps, "site");
          if (PICOMESH_IS_OK(delete_res))
            rollback_ok = 1;
          else
            picomesh_error_destroy(delete_res.error);
          SVC_CLOSE(acc_rb);
        }
        yheaders_free(del_caps);
      }
      if (!rollback_ok) {
        ywarn("authz: site bootstrap rollback FAILED for uid=%u (oauth) — "
              "username claim held; operator repair may be needed",
              uid);
        return PICOMESH_ERR(picomesh_void,
                            "ensure_oauth_account: registration failed and "
                            "could not be cleaned up — please retry");
      }
    }
    register_release_claim(uid, uname);
    return PICOMESH_ERR(picomesh_void,
                        "ensure_oauth_account: account create failed");
  }
  return PICOMESH_OK_VOID();
}

static struct picomesh_void_result
route_github_callback_post(struct loop_stream *stream, const char *headers_raw,
                           size_t headers_raw_len, const char *body,
                           size_t body_len, int keep_alive) {
  ...
  if (!header_value(headers_raw, headers_raw_len, "content-type", content_type,
                    sizeof(content_type))
           .value ||
      strcmp(content_type, "application/x-picoforge-oauth-relay") != 0)
    return send_json_error(
        stream, 403, "github callback: not an authorized relay", keep_alive);

  char code[512] = {0}, redirect_uri[1024] = {0}, state[256] = {0};
  if (!form_get(body, body_len, "code", code, sizeof(code)) ||
      !form_get(body, body_len, "redirect_uri", redirect_uri,
                sizeof(redirect_uri)) ||
      !code[0] || !redirect_uri[0])
    return send_json_error(stream, 400,
                           "github callback: missing code or redirect_uri",
                           keep_alive);
  ...
  if (!form_get(body, body_len, "state", state, sizeof(state)) || !state[0])
    return send_json_error(stream, 403, "github callback: missing OAuth state",
                           keep_alive);

  SVC_OPEN(github_svc, "github_authn", github_authn_github_authn_create);
  if (!github_svc.ok)
    return send_json_error(stream, 502, "github_authn unreachable", keep_alive);
  struct picomesh_json_result exchange_res =
      github_authn_github_authn_exchange_code(&github_svc.c, github_svc.obj,
                                              NULL, code, redirect_uri);
  SVC_CLOSE(github_svc);
  if (PICOMESH_IS_ERR(exchange_res)) {
    char msg[512];
    snprintf(msg, sizeof(msg), "github callback: %s",
             exchange_res.error.msg ? exchange_res.error.msg
                                    : "exchange failed");
  ...
  char uname[64] = {0};
  struct json_doc *doc =
      json_parse(exchange_res.value, strlen(exchange_res.value));
  if (doc) {
    const struct json_value *root = json_doc_root(doc);
    uid = (uint32_t)json_as_int(json_object_get(root, "uid"), 0);
    const char *username_str =
        json_as_string(json_object_get(root, "username"), NULL);
    if (username_str)
      snprintf(uname, sizeof(uname), "%s", username_str);
    json_doc_free(doc);
  }
  free(exchange_res.value);
  if (!uid || !username_ok(uname))
    return send_json_error(stream, 401, "github callback: invalid GitHub user",
                           keep_alive);
  ...
  if (PICOMESH_IS_ERR(account_res)) {
    ...
    snprintf(msg, sizeof(msg), "%s",
             account_res.error.msg ? account_res.error.msg
                                   : "github account link failed");
    picomesh_error_destroy(account_res.error);
    return send_json_error(stream, 403, msg, keep_alive);
  }
  char tok[64];
  struct picomesh_void_result session_res =
      oauth_start_session(uid, uname, tok, sizeof(tok));
  if (PICOMESH_IS_ERR(session_res)) {
    char msg[256];
    snprintf(msg, sizeof(msg), "%s",
             session_res.error.msg ? session_res.error.msg
                                   : "github session failed");
    picomesh_error_destroy(session_res.error);
    return send_json_error(stream, 500, msg, keep_alive);
  }
  char cookie_hdr[256];
  struct picomesh_void_result cookie_res =
      build_session_cookies(cookie_hdr, sizeof(cookie_hdr), tok, uname);
 ...
 * a credential yet. */
static struct picomesh_void_result route_login_post(struct loop_stream *stream,
                                                    const char *body,
                                                    size_t body_len,
                                                    int keep_alive) {
  ...
  /* Issue #29: resolve the ASSIGNED uid from the username instead of
   * recomputing fnv1a32(username). A 0 result means no confirmed account —
   * that is what differentiates /login from /register. This resolves both
   * assigned-uid accounts and legacy hash-derived ones (uid_for_username
   * returns whatever uid the `usernames` row records). A backend error fails
   * closed (login unavailable), never silently anonymous. */
  SVC_OPEN(accounts_svc, "accounts", accounts_accounts_create);
  if (!accounts_svc.ok) {
    render_login(stream, "accounts service unreachable", keep_alive);
    return PICOMESH_OK_VOID();
  }
  struct picomesh_int64_result uid_res = accounts_accounts_uid_for_username(
      &accounts_svc.c, accounts_svc.obj, NULL, uname);
  SVC_CLOSE(accounts_svc);
  if (PICOMESH_IS_ERR(uid_res)) {
    picomesh_error_print(stderr, "route_login_post: uid_for_username",
                         uid_res.error);
    picomesh_error_destroy(uid_res.error);
    render_login(stream, "login temporarily unavailable — try again",
                 keep_alive);
    return PICOMESH_OK_VOID();
  }
  if (uid_res.value <= 0) {
  ...
  uint32_t uid = (uint32_t)uid_res.value;

  const char *err = NULL;
  char tok[64];
  struct picomesh_int_result auth_res =
      auth_and_start_session(uid, uname, pw, tok, sizeof(tok), &err);
  PICOMESH_RETURN_IF_ERR(picomesh_void, auth_res,
                         "route_login_post: auth_and_start_session failed");
  if (!auth_res.value) {
  ...
  struct picomesh_void_result cookie_res =
      build_session_cookies(cookie_hdr, sizeof(cookie_hdr), tok, uname);
  if (PICOMESH_IS_ERR(cookie_res)) {
    picomesh_error_print(stderr, "route_login_post: build cookies",
                         cookie_res.error);
    picomesh_error_destroy(cookie_res.error);
  ...
  char landing[128];
  struct picomesh_void_result landing_res =
      landing_url(uid, uname, landing, sizeof(landing));
  PICOMESH_RETURN_IF_ERR(picomesh_void, landing_res,
                         "landing url resolution failed");
  send_redirect(stream, landing, keep_alive, cookie_hdr);
  return PICOMESH_OK_VOID();
}
...
static struct picomesh_void_result register_release_claim(uint32_t uid,
  ...
  if (!acc_rel.ok)
    return PICOMESH_OK_VOID();
  struct picomesh_int_result release_res = accounts_accounts_release_username(
      &acc_rel.c, acc_rel.obj, NULL, uid, uname);
  SVC_CLOSE(acc_rel);
  PICOMESH_RETURN_IF_ERR(picomesh_void, release_res,
                         "register_release_claim: release username failed");
  return PICOMESH_OK_VOID();
}
...
static struct picomesh_void_ptr_result internal_caps(uint32_t uid) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_OK(picomesh_void_ptr, NULL);
  struct picomesh_string_result secret = picomesh_security_jwt_secret(engine);
  PICOMESH_RETURN_IF_ERR(picomesh_void_ptr, secret,
                         "internal_caps: jwt secret unavailable");
  int64_t now = picomesh_security_now();
  char *claims = picomesh_jwt_build_claims(
      "picomesh", uid, "system", PICOMESH_GROUP_SYSTEM, now, now + 60);
  ...
  if (headers) {
    ...
    yheaders_set(headers, "jwt", jwt);
  }
  free(jwt);
  return PICOMESH_OK(picomesh_void_ptr, headers);
}
...
static struct picomesh_void_result
route_register_post(struct loop_stream *stream, const char *body,
                    size_t body_len, int keep_alive) {
  char uname[64] = {0}, pwtext[128] = {0};
  if (!form_get(body, body_len, "username", uname, sizeof(uname)) ||
      !form_get(body, body_len, "password", pwtext, sizeof(pwtext)) ||
      !username_ok(uname) || !*pwtext) {
    render_register(stream, "username (a-z, 0-9, ._-) and password required",
                    keep_alive);
    return PICOMESH_OK_VOID();
  }
  int64_t pw = hash_password(pwtext);

  /* Step 0 — allocate an ASSIGNED, never-reused uid (issue #29) instead of
   * uid = fnv1a32(username). This is the fix that retires the hash-collision
   * account-takeover risk: a fresh uid can never collide with a completed
   * account (allocate_uid refuses any id that already has a `users` row), and
   * because it is never reused, a crash anywhere downstream only strands a uid
   * no future registrant will be handed — so a partially-written credential
   * can never be inherited. Username uniqueness is still enforced separately by
   * the `usernames` claim (step 1), so two concurrent registrations of the same
   * name get different uids but only one wins the claim. A backend error fails
   * closed. */
  uint32_t uid;
  {
    SVC_OPEN(acc_alloc, "accounts", accounts_accounts_create);
    if (!acc_alloc.ok) {
      render_register(stream, "accounts service unreachable", keep_alive);
      return PICOMESH_OK_VOID();
    }
    struct picomesh_int64_result alloc_res =
        accounts_accounts_allocate_uid(&acc_alloc.c, acc_alloc.obj, NULL);
    SVC_CLOSE(acc_alloc);
    if (PICOMESH_IS_ERR(alloc_res)) {
      picomesh_error_print(stderr, "route_register_post: allocate_uid",
                           alloc_res.error);
      picomesh_error_destroy(alloc_res.error);
      render_register(stream,
                      "registration temporarily unavailable — try again",
                      keep_alive);
      return PICOMESH_OK_VOID();
    }
  ...
  if (PICOMESH_IS_ERR(claim_res)) {
    ...
    render_register(stream, "registration temporarily unavailable — try again",
                    keep_alive);
    return PICOMESH_OK_VOID();
  }
  if (claim_res.value != 1) {
  ...
  if (!pw_sv.ok) {
    register_release_claim(uid, uname);
    render_register(stream, "password_authn unreachable", keep_alive);
    return PICOMESH_OK_VOID();
  }
  struct picomesh_int_result pw_register_res =
      password_authn_password_authn_register(&pw_sv.c, pw_sv.obj, NULL, uid,
                                             pw);
  if (PICOMESH_IS_ERR(pw_register_res)) {
    ...
    register_release_claim(uid, uname);
    render_register(
        stream, "could not store credentials (backend not ready?) — try again",
        keep_alive);
    return PICOMESH_OK_VOID();
  }
  if (pw_register_res.value == 0) {
    struct picomesh_int_result change_res =
        password_authn_password_authn_change_password(&pw_sv.c, pw_sv.obj, NULL,
                                                      uid, pw);
    if (PICOMESH_IS_ERR(change_res)) {
      ...
      render_register(
          stream,
          "could not store credentials (backend not ready?) — try again",
          keep_alive);
      return PICOMESH_OK_VOID();
    }
  }
  ...
  if (PICOMESH_IS_ERR(personal_ns)) {
    ...
    register_release_claim(uid, uname);
    render_register(stream,
                    "username unavailable (reserved namespace) — try another",
                    keep_alive);
    return PICOMESH_OK_VOID();
  }
  free(personal_ns.value);
  ...
  int first_user = 0, created_site = 0;
  {
    ...
    if (first_user) {
      ...
      if (PICOMESH_IS_OK(site)) {
        ...
        struct picomesh_int64_result present = accounts_accounts_ns_resolve(
            &acc_boot.c, acc_boot.obj, NULL, "site");
        int site_present = PICOMESH_IS_OK(present) && present.value > 0;
        if (PICOMESH_IS_ERR(present))
          picomesh_error_destroy(present.error);
        if (!site_present) {
  ...
  struct picomesh_int_result account_register_res = accounts_accounts_register(
      &accounts_svc.c, accounts_svc.obj, NULL, uid, uname);
  SVC_CLOSE(accounts_svc);
  int was_new =
      PICOMESH_IS_OK(account_register_res) && account_register_res.value == 1;
  if (PICOMESH_IS_ERR(account_register_res))
    picomesh_error_destroy(account_register_res.error);
  if (!was_new) {
    if (created_site) {
      ...
      if (!rollback_ok) {
        ...
        ywarn("authz: site bootstrap rollback FAILED for uid=%u — username "
              "claim held; operator repair may be needed",
              uid);
        render_register(stream,
                        "registration failed and could not be cleaned up — "
                        "please retry, or contact the operator",
                        keep_alive);
        return PICOMESH_OK_VOID();
      }
    }
  ...
  const char *err = NULL;
  char tok[64];
  struct picomesh_int_result reg_auth_res =
      auth_and_start_session(uid, uname, pw, tok, sizeof(tok), &err);
  PICOMESH_RETURN_IF_ERR(picomesh_void, reg_auth_res,
                         "route_register_post: auth_and_start_session failed");
  if (!reg_auth_res.value) {
...
static struct picomesh_void_result route_logout(struct loop_stream *stream,
                                                const char *headers_raw,
                                                size_t headers_raw_len,
                                                int keep_alive) {
  char tok[64];
  if (cookie_get(headers_raw, headers_raw_len, "picomesh-sid", tok,
                 sizeof(tok)) &&
      tok[0]) {
    SVC_OPEN(session_svc, "session", session_session_create);
    if (session_svc.ok) {
      struct picomesh_int_result destroy_res =
          session_session_destroy(&session_svc.c, session_svc.obj, NULL, tok);
      SVC_CLOSE(session_svc);
      /* Best-effort: the cookie is cleared regardless, but a backend
       * failure to drop the server-side session is logged with its chain. */
      if (PICOMESH_IS_ERR(destroy_res)) {
        picomesh_error_print(stderr, "route_logout: session destroy",
                             destroy_res.error);
        picomesh_error_destroy(destroy_res.error);
      }
    }
  }
  send_redirect(
      stream, "/login", keep_alive,
...
static uint32_t hash_repo(const char *account, const char *name) {
  ...
  if (account)
    for (const unsigned char *cursor = (const unsigned char *)account; *cursor;
         ++cursor) {
      hash ^= *cursor;
      hash *= 16777619u;
    }
...
static int is_reserved_top(const char *seg, size_t seg_len) {
  ...
  for (size_t i = 0; RESERVED[i]; ++i) {
    ...
    if (entry_len == seg_len && memcmp(seg, RESERVED[i], entry_len) == 0)
      return 1;
  }
  return 0;
}
...
static struct picomesh_int_result repo_storage_open(struct svc_ctx *out) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_OK(picomesh_int, 0);
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, create_res,
                         "repo_storage_open: storage create failed");
...
static struct picomesh_int_result is_site_admin(uint32_t uid) {
  ...
  if (!accounts_svc.ok)
    return PICOMESH_OK(picomesh_int, 0);
  struct picomesh_string_result groups_res =
      accounts_accounts_groups(&accounts_svc.c, accounts_svc.obj, NULL, uid);
  SVC_CLOSE(accounts_svc);
  PICOMESH_RETURN_IF_ERR(picomesh_int, groups_res,
                         "is_site_admin: groups lookup failed");
  int admin =
      groups_res.value && picomesh_groups_max_role(groups_res.value, "site") >=
                              picomesh_role_rank("maintainer");
  free(groups_res.value);
  return PICOMESH_OK(picomesh_int, admin ? 1 : 0);
}
 ...
 */
static struct picomesh_uint32_result
repo_register(const char *account, const char *name, uint32_t owner_uid) {
  uint32_t repo_id = hash_repo(account, name);
  struct svc_ctx storage_svc = {0};
  struct picomesh_int_result open_res = repo_storage_open(&storage_svc);
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, open_res,
                         "repo_register: storage open failed");
  if (!open_res.value)
    return PICOMESH_OK(picomesh_uint32, 0);
  char key[96];
  snprintf(key, sizeof(key), "by_id:%u", repo_id);
  struct picomesh_int_result set_id = sharded_storage_db_set(
      &storage_svc.c, storage_svc.obj, NULL, "repos", key, "1");
  if (PICOMESH_IS_ERR(set_id)) {
    SVC_CLOSE(storage_svc);
    return PICOMESH_ERR(picomesh_uint32, "repo_register: by_id write failed",
                        set_id);
  }
  snprintf(key, sizeof(key), "owner:%u:count", owner_uid);
  struct picomesh_string_result current_res = sharded_storage_db_get(
      &storage_svc.c, storage_svc.obj, NULL, "repos", key);
  if (PICOMESH_IS_ERR(current_res)) {
    SVC_CLOSE(storage_svc);
    return PICOMESH_ERR(picomesh_uint32, "repo_register: count read failed",
                        current_res);
  }
  int64_t count =
      (current_res.value ? strtoll(current_res.value, NULL, 10) : 0) + 1;
  free(current_res.value);
  char count_buf[32];
  snprintf(count_buf, sizeof(count_buf), "%lld", (long long)count);
  struct picomesh_int_result set_count = sharded_storage_db_set(
      &storage_svc.c, storage_svc.obj, NULL, "repos", key, count_buf);
  if (PICOMESH_IS_ERR(set_count)) {
    SVC_CLOSE(storage_svc);
    return PICOMESH_ERR(picomesh_uint32, "repo_register: count write failed",
                        set_count);
  }
  SVC_CLOSE(storage_svc);
...
static struct picomesh_void_result
route_repos_new_post(struct loop_stream *stream, uint32_t uid,
                     const char *uname, const char *body, size_t body_len,
                     int keep_alive) {
  if (!uname || !*uname) {
    send_redirect(stream, "/login", keep_alive, NULL);
    return PICOMESH_OK_VOID();
  }
  ...
  if (repo_svc.ok && sys_caps) {
    struct picomesh_uint32_result make_res = git_repo_git_repo_make(
        &repo_svc.c, repo_svc.obj, sys_caps, uid, uname, name);
    made = PICOMESH_IS_OK(make_res) && make_res.value != 0;
    if (PICOMESH_IS_ERR(make_res)) {
      picomesh_error_print(stderr, "git_repo_make", make_res.error);
      picomesh_error_destroy(make_res.error);
    }
    SVC_CLOSE(repo_svc);
  } else if (repo_svc.ok) {
    SVC_CLOSE(repo_svc);
  ...
  struct picomesh_uint32_result repo_id_res = repo_register(uname, name, uid);
  if (PICOMESH_IS_ERR(repo_id_res)) {
    picomesh_error_print(stderr, "repo_register", repo_id_res.error);
    picomesh_error_destroy(repo_id_res.error);
    send_redirect(stream, "/repos", keep_alive, NULL);
    return PICOMESH_OK_VOID();
  }
  uint32_t repo_id = repo_id_res.value;
  if (!repo_id) {
    send_redirect(stream, "/repos", keep_alive, NULL);
    return PICOMESH_OK_VOID();
  }
 ...
 * /register.) */
static struct picomesh_void_result
route_admin_tokens_mint_pat(struct loop_stream *stream, const char *body,
                            size_t body_len, int keep_alive) {
  char uid_s[32];
  if (form_get(body, body_len, "uid", uid_s, sizeof(uid_s))) {
    uint32_t uid = (uint32_t)strtoul(uid_s, NULL, 10);
    SVC_OPEN(pat_svc, "personal_access_tokens",
             personal_access_tokens_personal_access_tokens_create);
    if (pat_svc.ok && uid) {
      struct picomesh_uint32_result mint_res =
          personal_access_tokens_personal_access_tokens_mint(
              &pat_svc.c, pat_svc.obj, NULL, uid);
      SVC_CLOSE(pat_svc);
      if (PICOMESH_IS_ERR(mint_res)) {
        picomesh_error_print(stderr, "route_admin_tokens_mint_pat: mint",
                             mint_res.error);
        picomesh_error_destroy(mint_res.error);
      }
    }
  }
...
static struct picomesh_void_result send_json_ex(struct loop_stream *stream,
                                                int status, const char *body,
                                                size_t body_len, int keep_alive,
                                                const char *extra_headers) {
  const char *reason = "OK";
  ...
  int written =
      snprintf(header, sizeof(header),
               ...
               extra_headers ? extra_headers : "");
  if (written <= 0)
    return PICOMESH_ERR(picomesh_void, "send_json_ex: header format failed");
  struct picomesh_size_result header_write =
      loop_write(stream, header, (size_t)written);
  if (PICOMESH_IS_ERR(header_write))
...
static struct picomesh_void_result send_json(struct loop_stream *stream,
                                             int status, const char *body,
                                             size_t body_len, int keep_alive) {
...
static struct picomesh_void_result send_json_error(struct loop_stream *stream,
                                                   ...
                                                   int keep_alive) {
  if (status >= 500)
    yerror("yhttp gateway request failed: %s", message ? message : "");
  struct json_writer *writer = json_writer_new();
  if (!writer)
    return PICOMESH_ERR(picomesh_void, "send_json_error: writer alloc failed");
  ...
  struct picomesh_void_result send_res =
      send_json(stream, status, data, len, keep_alive);
...
static struct picomesh_int_result extract_session_token(const char *headers_raw,
                                                        size_t headers_raw_len,
                                                        char *out, size_t cap) {
  if (cookie_get(headers_raw, headers_raw_len, "picomesh-sid", out, cap) &&
      out[0])
    return PICOMESH_OK(picomesh_int, 1);
  struct picomesh_int_result sid_hdr =
      header_value(headers_raw, headers_raw_len, "picomesh-sid", out, cap);
  PICOMESH_RETURN_IF_ERR(picomesh_int, sid_hdr,
                         "extract_session_token: sid header read failed");
  if (sid_hdr.value && out[0])
    return PICOMESH_OK(picomesh_int, 1);
  char auth[128];
  struct picomesh_int_result auth_hdr = header_value(
      headers_raw, headers_raw_len, "authorization", auth, sizeof(auth));
  PICOMESH_RETURN_IF_ERR(
      picomesh_int, auth_hdr,
      "extract_session_token: authorization header read failed");
  if (auth_hdr.value) {
...
static struct picomesh_uint32_result uid_for_token(const char *token) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, create_res,
                         "uid_for_token: session create failed");
  struct picomesh_uint32_result lookup_res =
      session_session_lookup(&ctx, create_res.value, NULL, token);
  object_release_in_ctx(&ctx, create_res.value);
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, lookup_res,
                         "uid_for_token: session lookup failed");
  return PICOMESH_OK(picomesh_uint32, lookup_res.value);
}
...
static struct picomesh_string_result session_jwt_for_token(const char *token) {
  if (!token || !*token)
  ...
  struct ctx ctx = picomesh_engine_service_ctx(engine, "session");
  struct object_ptr_result create_res = session_session_create(&ctx);
  PICOMESH_RETURN_IF_ERR(picomesh_string, create_res,
                         "session_jwt_for_token: session create failed");
  struct picomesh_string_result jwt_res =
      session_session_jwt(&ctx, create_res.value, NULL, token);
  object_release_in_ctx(&ctx, create_res.value);
  PICOMESH_RETURN_IF_ERR(picomesh_string, jwt_res,
                         "session_jwt_for_token: session jwt failed");
  if (jwt_res.value && jwt_res.value[0])
    return PICOMESH_OK(picomesh_string, jwt_res.value);
  free(jwt_res.value);
  return PICOMESH_OK(picomesh_string, NULL);
}
...
resolve_authctx(const char *headers_raw, size_t headers_raw_len,
  ...
  char token[64];
  struct picomesh_int_result token_res =
      extract_session_token(headers_raw, headers_raw_len, token, sizeof(token));
  PICOMESH_RETURN_IF_ERR(picomesh_void, token_res,
                         "resolve_authctx: token extract failed");
  if (!token_res.value)
    return PICOMESH_OK_VOID();
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
  ...
  struct ctx ctx = picomesh_engine_service_ctx(engine, "session");
  struct object_ptr_result create_res = session_session_create(&ctx);
  PICOMESH_RETURN_IF_ERR(picomesh_void, create_res,
                         "resolve_authctx: session create failed");
  struct picomesh_string_result jwt_res =
      session_session_jwt(&ctx, create_res.value, NULL, token);
  object_release_in_ctx(&ctx, create_res.value);
  PICOMESH_RETURN_IF_ERR(picomesh_void, jwt_res,
                         "resolve_authctx: session jwt failed");
  if (jwt_res.value && jwt_res.value[0]) {
    struct picomesh_string_result secret = picomesh_security_jwt_secret(engine);
    if (PICOMESH_IS_OK(secret)) {
      struct picomesh_void_result ctx_res =
          picomesh_authctx_from_jwt(jwt_res.value, secret.value, out);
      if (PICOMESH_IS_ERR(ctx_res))
        picomesh_error_destroy(ctx_res.error);
      free(secret.value);
    } else {
...
static struct picomesh_void_ptr_result
gateway_security_get(struct picomesh_engine *engine) {
  struct gateway_security *security = picomesh_engine_worker_security(engine);
  if (security)
    return PICOMESH_OK(picomesh_void_ptr, security);

  security = calloc(1, sizeof(*security));
  if (!security)
    return PICOMESH_ERR(picomesh_void_ptr,
                        "gateway_security_get: out of memory");

  const struct config *config = picomesh_engine_config(engine);
  struct config_node_ptr_result authn_res =
      config ? config_get(config, "security.authenticators")
             : (struct config_node_ptr_result){.ok = 0};
  if (config && PICOMESH_IS_ERR(authn_res)) {
    /* A config READ/parse failure must never silently downgrade the gateway
     * to unsecured (which would authorize every request). Treat it as a hard
     * misconfiguration: mark secured with build_ok unset so every request
     * reports the 500 path below, exactly like a build failure. Genuine
     * absence of `security.authenticators` (OK with NULL value, or no config
     * at all) is the only case that intentionally runs unsecured. */
    picomesh_error_print(stderr, "gateway_security_get: authenticators config",
                         authn_res.error);
    picomesh_error_destroy(authn_res.error);
    security->secured = 1;
    security->build_ok = 0;
    picomesh_engine_worker_set_security(engine, security,
                                        gateway_security_free);
    return PICOMESH_OK(picomesh_void_ptr, security);
  }
  const struct config_node *authn_list =
      PICOMESH_IS_OK(authn_res) ? authn_res.value : NULL;
  security->secured = authn_list && config_node_kind(authn_list) == CONFIG_LIST;

  if (security->secured) {
    /* A build failure is cached as secured && !build_ok and reported per
     * request; render its full chain here so the misconfig is diagnosable. */
    struct picomesh_void_ptr_result chain_res =
        picomesh_authn_chain_build(engine, authn_list);
    if (PICOMESH_IS_OK(chain_res)) {
      security->chain = chain_res.value;
      struct config_node_ptr_result authz_res =
          config_get(config, "security.authorizer");
      if (PICOMESH_IS_ERR(authz_res)) {
        /* As with the authenticators read above, a config READ failure
         * here must not silently fall back to a default authorizer.
         * Leave build_ok unset so the request path reports the 500. */
        picomesh_error_print(stderr, "gateway_security_get: authorizer config",
                             authz_res.error);
        picomesh_error_destroy(authz_res.error);
      } else {
        struct picomesh_void_ptr_result authorizer_res =
            picomesh_authorizer_build(engine, authz_res.value);
        if (PICOMESH_IS_OK(authorizer_res)) {
          security->authorizer = authorizer_res.value;
          security->build_ok = 1;
        } else {
          picomesh_error_print(stderr, "gateway_security_get: authorizer build",
                               authorizer_res.error);
          picomesh_error_destroy(authorizer_res.error);
        }
      }
    } else {
      picomesh_error_print(stderr, "gateway_security_get: authn chain build",
                           chain_res.error);
...
static struct picomesh_void_result
route_json_rpc(struct loop_stream *stream, const char *headers_raw,
               size_t headers_raw_len, const char *body, size_t body_len,
               int keep_alive) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine) {
    send_json_error(stream, 500, "_rpc: no engine", keep_alive);
    return PICOMESH_OK_VOID();
  }

  struct json_doc *doc = json_parse(body, body_len);
  if (!doc) {
    send_json_error(stream, 400, "_rpc: invalid JSON", keep_alive);
    return PICOMESH_OK_VOID();
  }
  const struct json_value *root = json_doc_root(doc);
  const char *path = json_as_string(json_object_get(root, "path"), NULL);
  ...
  struct picomesh_service_call_result call_res =
      picomesh_resolve_service_call(engine, path);
  if (PICOMESH_IS_ERR(call_res)) {
    const char *resolve_msg =
        call_res.error.msg ? call_res.error.msg : "resolve failed";
    int code = strstr(resolve_msg, "bad path")     ? 400
               : strstr(resolve_msg, "not active") ? 404
                                                   : 502;
    char msg[320];
  ...
  struct picomesh_void_ptr_result security_res = gateway_security_get(engine);
  if (PICOMESH_IS_ERR(security_res)) {
    char errbuf[8192];
    picomesh_error_snprint(errbuf, sizeof(errbuf), security_res.error);
    picomesh_error_print(stderr, "route_json_rpc: gateway_security_get",
                         security_res.error);
    picomesh_error_destroy(security_res.error);
    picomesh_service_call_release(&call);
    json_doc_free(doc);
    send_json_error(stream, 500, errbuf, keep_alive);
    return PICOMESH_OK_VOID();
  }
  struct gateway_security *security = security_res.value;
  if (!security) {
    picomesh_service_call_release(&call);
    json_doc_free(doc);
    send_json_error(stream, 500, "_rpc: security pipeline unavailable",
                    keep_alive);
    return PICOMESH_OK_VOID();
  }
  ...
  if (security->secured) {
    if (!security->build_ok) {
      picomesh_service_call_release(&call);
      json_doc_free(doc);
      send_json_error(stream, 500,
                      "_rpc: security config error (authenticators/authorizer)",
                      keep_alive);
      return PICOMESH_OK_VOID();
    }
    ...
    struct picomesh_authn_outcome_result outcome_res =
        picomesh_authn_chain_run(security->chain, &authn_req);
    if (PICOMESH_IS_ERR(outcome_res)) {
      /* Infrastructure failure in the authn chain: render the full cause
       * chain to both stderr and the 500 body. */
      char eb[8192];
      picomesh_error_snprint(eb, sizeof(eb), outcome_res.error);
      picomesh_error_print(stderr, "_rpc: authn chain", outcome_res.error);
      picomesh_error_destroy(outcome_res.error);
      picomesh_service_call_release(&call);
      json_doc_free(doc);
      send_json_error(stream, 500, eb, keep_alive);
      return PICOMESH_OK_VOID();
    }
    struct picomesh_authn_outcome outcome = outcome_res.value;
    if (picomesh_authn_outcome_failed(&outcome)) {
      char msg[288];
      snprintf(msg, sizeof(msg), "_rpc: %s",
               outcome.error ? outcome.error : "invalid credentials");
    ...
    struct picomesh_authz_decision_result decision_res =
        picomesh_authorizer_decide(security->authorizer, path, args,
                                   outcome.jwt);
    if (PICOMESH_IS_ERR(decision_res)) {
      /* Infrastructure failure deciding authz: render the full cause
       * chain to both stderr and the 500 body. */
      char eb[8192];
      picomesh_error_snprint(eb, sizeof(eb), decision_res.error);
      picomesh_error_print(stderr, "_rpc: authz decide", decision_res.error);
      picomesh_error_destroy(decision_res.error);
      picomesh_authn_outcome_free(&outcome);
      picomesh_service_call_release(&call);
      json_doc_free(doc);
      send_json_error(stream, 500, eb, keep_alive);
    ...
    struct picomesh_authz_decision decision = decision_res.value;
    if (!decision.allowed) {
      char msg[288];
      snprintf(msg, sizeof(msg), "_rpc: %s",
               decision.reason[0] ? decision.reason : "forbidden");
      picomesh_authn_outcome_free(&outcome);
      picomesh_service_call_release(&call);
      json_doc_free(doc);
      send_json_error(stream, decision.status ? decision.status : 403, msg,
                      keep_alive);
      return PICOMESH_OK_VOID();
    }
    ...
    if (outcome.jwt) {
      ...
      struct picomesh_string_result secret =
          picomesh_security_jwt_secret(engine);
      if (PICOMESH_IS_OK(secret)) {
        struct picomesh_authctx claims;
        struct picomesh_void_result ctx_res =
            picomesh_authctx_from_jwt(outcome.jwt, secret.value, &claims);
        if (PICOMESH_IS_OK(ctx_res) && claims.authenticated)
          uid = claims.uid;
        else if (PICOMESH_IS_ERR(ctx_res))
          picomesh_error_destroy(ctx_res.error);
        free(secret.value);
      } else {
        picomesh_error_destroy(secret.error);
    ...
    char token[64];
    struct picomesh_int_result token_res = extract_session_token(
        headers_raw, headers_raw_len, token, sizeof(token));
    if (PICOMESH_IS_ERR(token_res)) {
      picomesh_error_print(stderr, "_rpc: extract session token",
                           token_res.error);
      picomesh_error_destroy(token_res.error);
    } else if (token_res.value) {
      struct picomesh_string_result jwt_res = session_jwt_for_token(token);
      verified_jwt = PICOMESH_IS_OK(jwt_res) ? jwt_res.value : NULL;
      if (PICOMESH_IS_ERR(jwt_res)) {
        picomesh_error_print(stderr, "_rpc: session jwt lookup", jwt_res.error);
        picomesh_error_destroy(jwt_res.error);
      }
      if (verified_jwt) {
        struct picomesh_string_result secret =
            picomesh_security_jwt_secret(engine);
        ...
        struct picomesh_uint32_result uid_res = uid_for_token(token);
        if (PICOMESH_IS_OK(uid_res)) {
          uid = uid_res.value;
        } else {
          picomesh_error_print(stderr, "_rpc: uid_for_token", uid_res.error);
          picomesh_error_destroy(uid_res.error);
        }
      }
    }
  ...
  if (headers) {
    yheaders_set_u32(headers, "uid", uid);
    if (verified_jwt)
      yheaders_set(headers, "jwt", verified_jwt);
    char traceparent[128] = {0};
    header_value(headers_raw, headers_raw_len, "traceparent", traceparent,
                 sizeof(traceparent));
  ...
  struct picomesh_void_result invoke_res =
      invoke_fn(&call.ctx, call.obj, headers, args, writer, err, sizeof(err));
  int invoke_failed = PICOMESH_IS_ERR(invoke_res);
  if (headers)
    ytelemetry_span_end(&span, !invoke_failed, invoke_failed ? err : NULL);
  ...
  if (invoke_failed) {
    ...
    json_writer_free(writer);
    json_doc_free(doc);
    send_json_error(stream, 500, err[0] ? err : "_rpc: call failed",
                    keep_alive);
    return PICOMESH_OK_VOID();
  }
  json_writer_end_object(writer);
  ...
  if (span.trace_id[0]) {
    char tpval[64];
    ytelemetry_traceparent_format(tpval, sizeof(tpval), span.trace_id,
                                  span.span_id, span.sampled);
    snprintf(tp_header, sizeof(tp_header), "traceparent: %s\r\n", tpval);
  }
  send_json_ex(stream, 200, data, len, keep_alive,
               tp_header[0] ? tp_header : NULL);
  json_writer_free(writer);
  json_doc_free(doc);
  return PICOMESH_OK_VOID();
}
...
static struct picomesh_void_result route_whoami(struct loop_stream *stream,
                                                const char *headers_raw,
                                                size_t headers_raw_len,
                                                int keep_alive) {
  ...
  struct picomesh_void_result authctx_res =
      resolve_authctx(headers_raw, headers_raw_len, &caller);
  PICOMESH_RETURN_IF_ERR(picomesh_void, authctx_res,
                         "route_whoami: resolve authctx failed");
  int admin =
      caller.uid && picomesh_groups_max_role(caller.groups_csv, "site") >=
                        picomesh_role_rank("maintainer");

  struct json_writer *writer = json_writer_new();
  json_writer_begin_object(writer);
  json_writer_key(writer, "uid");
  json_writer_int(writer, (int64_t)caller.uid);
  json_writer_key(writer, "username");
  json_writer_string(writer, caller.username);
  json_writer_key(writer, "is_admin");
  json_writer_bool(writer, admin);
  ...
  json_writer_key(writer, "namespaces");
  json_writer_begin_array(writer);
  for (const char *cursor = caller.groups_csv; cursor && *cursor;) {
    ...
    if (colon) {
      ...
      if (path_len < sizeof(ns_path) && role_len < sizeof(ns_role)) {
        memcpy(ns_path, cursor, path_len);
        ns_path[path_len] = 0;
        memcpy(ns_role, colon + 1, role_len);
        ns_role[role_len] = 0;
        json_writer_begin_object(writer);
        json_writer_key(writer, "path");
        json_writer_string(writer, ns_path);
        json_writer_key(writer, "role");
        json_writer_string(writer, ns_role);
        json_writer_end_object(writer);
      }
    }
    if (!comma)
 ...
 * of the dotted path the console feeds back to /_rpc and
 * /<svc.class>/_describe. */
...
static void describe_method_cb(const char *name, method_slot slot, void *ud) {
  (void)slot;
  json_writer_string((struct json_writer *)ud, name);
}

/* class_for_each callback — its void signature is fixed by the iterator's cb
 * type, so a slot-enumeration failure is absorbed here (chain rendered). */
PICOMESH_EXTERNAL_CALLBACK
static void describe_class_cb(const struct class *cls, const char *qname,
                              void *ud) {
  struct describe_class_ctx *class_ctx = ud;
  if (strncmp(qname, class_ctx->prefix, class_ctx->prefix_len) != 0)
    return;
  ...
  json_writer_begin_object(class_ctx->w);
  json_writer_key(class_ctx->w, "class");
  json_writer_string(class_ctx->w, dotpath);
  json_writer_key(class_ctx->w, "qname");
  json_writer_string(class_ctx->w, qname);
  json_writer_key(class_ctx->w, "methods");
  json_writer_begin_array(class_ctx->w);
  struct picomesh_void_result slots_res =
      class_for_each_slot(cls, describe_method_cb, class_ctx->w);
  if (PICOMESH_IS_ERR(slots_res)) {
    picomesh_error_print(stderr, "describe_class_cb: class_for_each_slot",
                         slots_res.error);
    picomesh_error_destroy(slots_res.error);
  }
  json_writer_end_array(class_ctx->w);
  json_writer_end_object(class_ctx->w);
}
...
static void describe_emit_service(struct describe_services_ctx *services_ctx,
                                  const char *name) {
  if (!name || !*name)
    return;
  for (size_t i = 0; i < services_ctx->seen_n; ++i)
    if (strcmp(services_ctx->seen[i], name) == 0)
      return;
  if (services_ctx->seen_n <
      sizeof(services_ctx->seen) / sizeof(services_ctx->seen[0]))
    snprintf(services_ctx->seen[services_ctx->seen_n++],
             sizeof(services_ctx->seen[0]), "%s", name);
  int remote = services_ctx->e &&
               picomesh_engine_service_ctx(services_ctx->e, name).peer != NULL;
  json_writer_begin_object(services_ctx->w);
  json_writer_key(services_ctx->w, "service");
  json_writer_string(services_ctx->w, name);
  json_writer_key(services_ctx->w, "source");
  json_writer_string(services_ctx->w, remote ? "remote" : "local");
  json_writer_key(services_ctx->w, "classes");
  json_writer_begin_array(services_ctx->w);
  char prefix[80];
  ...
  json_writer_end_array(services_ctx->w);
  json_writer_end_object(services_ctx->w);
}
...
static void describe_emit_plugins(struct describe_services_ctx *services_ctx,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    describe_emit_service(services_ctx,
                          entry ? config_node_as_string(entry, NULL) : NULL);
  }
}

/* for_each cb: capture one `remotes[]` map entry's `service:` value. */
static int describe_grab_service_cb(const char *key,
                                    const struct config_node *val, void *ud) {
  if (strcmp(key, "service") == 0) {
    const char *service_str = config_node_as_string(val, NULL);
    if (service_str) {
      snprintf((char *)ud, 64, "%s", service_str);
      return 1; /* stop */
    }
  }
  return 0;
...
static void describe_emit_remotes(struct describe_services_ctx *services_ctx,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    config_node_for_each(entry, describe_grab_service_cb, remote_name);
    if (remote_name[0])
      describe_emit_service(services_ctx, remote_name);
  }
}
...
static struct config_node_ptr_result
describe_node_list(struct picomesh_engine *engine, const char *named_suffix,
                   const char *top_key) {
  const struct config *cfg = picomesh_engine_config(engine);
  if (!cfg)
    return PICOMESH_OK(config_node_ptr, NULL);
  struct argv_chain *cli_chain = picomesh_engine_cli(engine);
  const char *name =
      cli_chain ? argv_get_string(cli_chain, "name", NULL) : NULL;
  if (name && *name) {
    char path[256];
    snprintf(path, sizeof(path), "mesh.services.%s.%s", name, named_suffix);
    struct config_node_ptr_result named_res = config_get(cfg, path);
    PICOMESH_RETURN_IF_ERR(config_node_ptr, named_res,
                           "describe_node_list: named config read failed");
    if (named_res.value)
      return named_res;
  }
  struct config_node_ptr_result top_res = config_get(cfg, top_key);
  PICOMESH_RETURN_IF_ERR(config_node_ptr, top_res,
                         "describe_node_list: top config read failed");
  return top_res;
}
...
static struct picomesh_void_result route_describe(struct loop_stream *stream,
                                                  const char *class_dotpath,
  ...
  if (class_dotpath && *class_dotpath) {
    ...
    for (char *cursor = class_qname; *cursor; ++cursor)
      if (*cursor == '.')
        *cursor = '_';
    ...
    if (PICOMESH_IS_OK(class_res) && class_res.value) {
      ...
      json_writer_begin_object(writer);
      json_writer_key(writer, "path");
      json_writer_string(writer, class_dotpath);
      json_writer_key(writer, "class");
      json_writer_string(writer, class_qname);
      ...
      if (!params) {
        json_writer_free(writer);
        send_json_error(stream, 404, "_describe: unknown class or method",
                        keep_alive);
        return PICOMESH_OK_VOID();
      }
      json_writer_begin_object(writer);
      json_writer_key(writer, "path");
      json_writer_string(writer, class_dotpath);
      json_writer_key(writer, "method");
      json_writer_string(writer, class_qname);
      json_writer_key(writer, "params");
      json_writer_begin_array(writer);
      for (size_t i = 0; i < params->count; ++i) {
        json_writer_begin_object(writer);
        json_writer_key(writer, "name");
        json_writer_string(writer, params->items[i].name);
        json_writer_key(writer, "type");
        json_writer_string(writer, params->items[i].type);
        json_writer_end_object(writer);
      }
      json_writer_end_array(writer);
      json_writer_end_object(writer);
    ...
    json_writer_begin_array(writer);
    if (engine) {
      struct describe_services_ctx services_ctx = {
          .e = engine, .w = writer, .seen_n = 0};
      struct config_node_ptr_result plugins_list =
          describe_node_list(engine, "plugins", "plugins");
      if (PICOMESH_IS_ERR(plugins_list)) {
        picomesh_error_print(stderr, "describe plugins", plugins_list.error);
        picomesh_error_destroy(plugins_list.error);
      } else
        describe_emit_plugins(&services_ctx, plugins_list.value);
      struct config_node_ptr_result remotes_list =
          describe_node_list(engine, "config.remotes", "remotes");
      if (PICOMESH_IS_ERR(remotes_list)) {
        picomesh_error_print(stderr, "describe remotes", remotes_list.error);
        picomesh_error_destroy(remotes_list.error);
      } else
        describe_emit_remotes(&services_ctx, remotes_list.value);
    }
    json_writer_end_array(writer);
    json_writer_key(writer, "tree");
    json_writer_bool(writer, tree ? true : false);
    json_writer_end_object(writer);
  }
...
static struct picomesh_int_result
try_hierarchical_describe(struct loop_stream *stream, const char *path,
                          int keep_alive) {
  const char *query = strchr(path, '?');
  ...
  int tree = 0;
  size_t suffix_len = 0;
  if (path_len > tree_suffix_len && memcmp(path + path_len - tree_suffix_len,
                                           tree_suffix, tree_suffix_len) == 0) {
    tree = 1;
    suffix_len = tree_suffix_len;
  } else if (path_len > desc_suffix_len &&
             memcmp(path + path_len - desc_suffix_len, desc_suffix,
                    desc_suffix_len) == 0) {
    tree = 0;
    suffix_len = desc_suffix_len;
  } else {
    return PICOMESH_OK(picomesh_int, 0);
  }
  ...
  if (dot_len == 0 || path[0] != '/')
    return PICOMESH_OK(picomesh_int, 0);
  char dotpath[160];
  if (dot_len - 1 >= sizeof(dotpath))
    return PICOMESH_OK(picomesh_int, 0);
  memcpy(dotpath, path + 1, dot_len - 1);
  dotpath[dot_len - 1] = 0;
  struct picomesh_void_result describe_res =
      route_describe(stream, dotpath, tree, keep_alive);
  PICOMESH_RETURN_IF_ERR(picomesh_int, describe_res,
                         "try_hierarchical_describe: route_describe failed");
  return PICOMESH_OK(picomesh_int, 1);
}
...
static struct picomesh_int_result
yhttp_node_has_remotes(struct picomesh_engine *engine) {
  struct config_node_ptr_result remotes_res =
      describe_node_list(engine, "config.remotes", "remotes");
  PICOMESH_RETURN_IF_ERR(picomesh_int, remotes_res,
                         "yhttp_node_has_remotes: remotes config read failed");
  const struct config_node *remotes_node = remotes_res.value;
  return PICOMESH_OK(picomesh_int,
                     remotes_node &&
                             config_node_kind(remotes_node) == CONFIG_LIST &&
                             config_node_size(remotes_node) > 0
                         ? 1
                         : 0);
}

struct picomesh_int_result
yhttp_frontend_try(struct loop_stream *stream, const char *method,
                   const char *path, const char *headers_raw,
                   size_t headers_raw_len, const char *body, size_t body_len,
                   int keep_alive) {
  ...
  if (!engine)
    return PICOMESH_OK(picomesh_int, 0);
  int has_session_remote =
      picomesh_engine_service_ctx(engine, "session").peer != NULL;
  struct config_node_ptr_result serve_app_res =
      config_get(picomesh_engine_config(engine), "yhttp.serve_app");
  PICOMESH_RETURN_IF_ERR(
      picomesh_int, serve_app_res,
      "yhttp_frontend_try: yhttp.serve_app config read failed");
  int serve_app =
      serve_app_res.value && config_node_as_bool(serve_app_res.value, 0);
  ...
  struct config_node_ptr_result bridge_only_res =
      config_get(picomesh_engine_config(engine), "yhttp.bridge_only");
  PICOMESH_RETURN_IF_ERR(
      picomesh_int, bridge_only_res,
      "yhttp_frontend_try: yhttp.bridge_only config read failed");
  int bridge_only =
      bridge_only_res.value && config_node_as_bool(bridge_only_res.value, 0);
  ...
  struct picomesh_int_result has_remotes_res = yhttp_node_has_remotes(engine);
  if (PICOMESH_IS_ERR(has_remotes_res)) {
    picomesh_error_print(stderr, "yhttp_node_has_remotes",
                         has_remotes_res.error);
    picomesh_error_destroy(has_remotes_res.error);
    return PICOMESH_OK(picomesh_int, 0);
  }
  int is_bridge = !is_gateway && has_remotes_res.value;
  if (!is_gateway && !is_bridge)
    return PICOMESH_OK(picomesh_int, 0);
   ...
   * the browser sidecar, MCP, CLI and tests use. */
  {
    ...
    if (is_post && path_len == 5 && memcmp(path, "/_rpc", 5) == 0) {
      if (query && strstr(query, "op="))
        return PICOMESH_OK(picomesh_int, 0);
      route_json_rpc(stream, headers_raw, headers_raw_len, body, body_len,
                     keep_alive);
      return PICOMESH_OK(picomesh_int, 1);
    }
    ...
    if (is_get || is_post) {
      if (path_eq(path, "/_describe")) {
        struct picomesh_void_result r =
            route_describe(stream, NULL, 0, keep_alive);
        if (PICOMESH_IS_ERR(r)) {
          char eb[8192];
          picomesh_error_snprint(eb, sizeof(eb), r.error);
          picomesh_error_print(stderr, "route_describe", r.error);
          picomesh_error_destroy(r.error);
          send_json_error(stream, 500, eb, keep_alive);
        }
        return PICOMESH_OK(picomesh_int, 1);
      }
      if (path_eq(path, "/_describe_tree")) {
        struct picomesh_void_result r =
            route_describe(stream, NULL, 1, keep_alive);
        if (PICOMESH_IS_ERR(r)) {
          char eb[8192];
          picomesh_error_snprint(eb, sizeof(eb), r.error);
          picomesh_error_print(stderr, "route_describe", r.error);
      ...
      if (path_eq(path, "/_whoami")) {
        struct picomesh_void_result r =
            route_whoami(stream, headers_raw, headers_raw_len, keep_alive);
        if (PICOMESH_IS_ERR(r)) {
          char eb[8192];
          picomesh_error_snprint(eb, sizeof(eb), r.error);
          picomesh_error_print(stderr, "route_whoami", r.error);
          picomesh_error_destroy(r.error);
          send_json_error(stream, 500, eb, keep_alive);
        }
        return PICOMESH_OK(picomesh_int, 1);
      }
      struct picomesh_int_result hd =
          try_hierarchical_describe(stream, path, keep_alive);
      if (PICOMESH_IS_ERR(hd)) {
        picomesh_error_print(stderr, "try_hierarchical_describe", hd.error);
        picomesh_error_destroy(hd.error);
      } else if (hd.value)
        return PICOMESH_OK(picomesh_int, 1);
    }
    ...
    if (is_get && path_eq(path, "/_trace")) {
      const char *moved =
          ...
          "  - a request's trace     -> GET /traces/<trace_id> on the trace "
          "collector\n";
    ...
    if (is_post && (path_eq(path, "/create") || path_eq(path, "/invoke"))) {
      send_json_error(stream, 410,
                      "removed: use POST /_rpc "
                      "{\"path\":\"service.class.method\",\"args\":[]}",
                      keep_alive);
      return PICOMESH_OK(picomesh_int, 1);
    }
    if (is_get && path_len == 9 && memcmp(path, "/describe", 9) == 0) {
      send_json_error(stream, 410,
                      "removed: use GET /<service.class>/_describe",
                      keep_alive);
      return PICOMESH_OK(picomesh_int, 1);
    }
  }
  ...
  if (is_post && path_eq(path, "/login")) {
    struct picomesh_void_result r =
        route_login_post(stream, body, body_len, keep_alive);
    if (PICOMESH_IS_ERR(r)) {
      char eb[8192];
      picomesh_error_snprint(eb, sizeof(eb), r.error);
      picomesh_error_print(stderr, "route_login_post", r.error);
      picomesh_error_destroy(r.error);
      send_json_error(stream, 500, eb, keep_alive);
    }
    return PICOMESH_OK(picomesh_int, 1);
  ...
  if (is_post && path_eq(path, "/auth/github/callback")) {
    struct picomesh_void_result route_res = route_github_callback_post(
        stream, headers_raw, headers_raw_len, body, body_len, keep_alive);
    if (PICOMESH_IS_ERR(route_res)) {
      yerror("yhttp: %s",
             route_res.error.msg ? route_res.error.msg : "route failed");
      picomesh_error_destroy(route_res.error);
    }
    return PICOMESH_OK(picomesh_int, 1);
  }
  if (is_post && path_eq(path, "/register")) {
    struct picomesh_void_result r =
        route_register_post(stream, body, body_len, keep_alive);
    if (PICOMESH_IS_ERR(r)) {
      char eb[8192];
      picomesh_error_snprint(eb, sizeof(eb), r.error);
      picomesh_error_print(stderr, "route_register_post", r.error);
      picomesh_error_destroy(r.error);
      send_json_error(stream, 500, eb, keep_alive);
    }
    return PICOMESH_OK(picomesh_int, 1);
  }
  if (is_post && path_eq(path, "/logout")) {
    struct picomesh_void_result r =
        route_logout(stream, headers_raw, headers_raw_len, keep_alive);
    if (PICOMESH_IS_ERR(r)) {
      char eb[8192];
      picomesh_error_snprint(eb, sizeof(eb), r.error);
      picomesh_error_print(stderr, "route_logout", r.error);
      picomesh_error_destroy(r.error);
      send_json_error(stream, 500, eb, keep_alive);
    }
    return PICOMESH_OK(picomesh_int, 1);
  ...
  if (uid) {
    ...
    if (is_post && path_eq(path, "/repos/new")) {
      struct picomesh_void_result r =
          route_repos_new_post(stream, uid, uname, body, body_len, keep_alive);
      if (PICOMESH_IS_ERR(r)) {
        char eb[8192];
        picomesh_error_snprint(eb, sizeof(eb), r.error);
        picomesh_error_print(stderr, "route_repos_new_post", r.error);
        picomesh_error_destroy(r.error);
        send_json_error(stream, 500, eb, keep_alive);
      }
      return PICOMESH_OK(picomesh_int, 1);
    ...
    if (strncmp(path, "/admin/", 7) == 0 && site_admin) {
      ...
      if (is_post && path_eq(path, "/admin/tokens/mint_pat")) {
        struct picomesh_void_result r =
            route_admin_tokens_mint_pat(stream, body, body_len, keep_alive);
        if (PICOMESH_IS_ERR(r)) {
          char eb[8192];
          picomesh_error_snprint(eb, sizeof(eb), r.error);
          picomesh_error_print(stderr, "route_admin_tokens_mint_pat", r.error);
          picomesh_error_destroy(r.error);
          send_json_error(stream, 500, eb, keep_alive);
        }
        return PICOMESH_OK(picomesh_int, 1);
