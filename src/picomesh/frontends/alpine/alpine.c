...
static const char ALPINE_CONSOLE_HTML[] =
    ...
    "<style>\n"
    " :root{--bg:#0b1014;--lift:#141a1f;--row:#1e262c;--bd:#364a47;--fg:#"
    "e0e5e4;\n"
    "       "
    "--mut:#9fa7a8;--faint:#556162;--acc:#6ba892;--acc2:#74c5a5;--err:#e06c6c}"
    "\n"
    " *{box-sizing:border-box}\n"
    " body{margin:0;font:14px/1.5 "
    "system-ui,sans-serif;background:var(--bg);color:var(--fg);height:100vh;"
    "display:flex;flex-direction:column}\n"
    " header{padding:9px 16px;border-bottom:1px solid "
    "var(--bd);display:flex;gap:10px;align-items:center}\n"
    " header h1{font-size:14px;margin:0;color:var(--acc)}\n"
    " header "
    ".up{color:var(--faint);font-size:12px;margin-left:auto;font-family:ui-"
    "monospace,monospace}\n"
    " button{background:var(--row);color:var(--fg);border:1px solid "
    "var(--bd);border-radius:4px;padding:4px 9px;cursor:pointer;font:inherit}\n"
    " button:hover{border-color:var(--acc)}\n"
    " main{flex:1;display:flex;min-height:0}\n"
    " #tree{width:38%;max-width:460px;overflow:auto;padding:10px "
    "8px;border-right:1px solid var(--bd)}\n"
    " #panel{flex:1;overflow:auto;padding:16px 20px}\n"
    " .svc>.hd,.cls>.hd{display:flex;gap:6px;align-items:center;cursor:pointer;"
    "padding:3px 4px;border-radius:4px}\n"
    " .svc>.hd:hover,.cls>.hd:hover{background:var(--lift)}\n"
    " .svc>.hd{font-weight:600}\n"
    " .cls{margin-left:14px}\n"
    " .cls>.hd{color:var(--mut);font-size:13px;font-family:ui-monospace,"
    "monospace}\n"
    " .tw{width:12px;display:inline-block;color:var(--faint)}\n"
    " .badge{font-size:10px;color:var(--faint);border:1px solid "
    "var(--bd);border-radius:3px;padding:0 5px}\n"
    " .meths{margin-left:26px;display:none}\n"
    " .meths.open{display:block}\n"
    " .cls.closed>.meths,.svc.closed>.body{display:none}\n"
    " .m{display:block;width:100%;text-align:left;border:0;background:none;"
    "color:var(--fg);\n"
    "    font-family:ui-monospace,monospace;font-size:12.5px;padding:2px "
    "6px;border-radius:4px;cursor:pointer}\n"
    " .m:hover{background:var(--lift)}\n"
    " .m.sel{background:var(--row);color:var(--acc2)}\n"
    " h2.path{font-family:ui-monospace,monospace;color:var(--acc);font-size:"
    "16px;margin:0 0 4px;word-break:break-all}\n"
    ...
    " .field input{width:100%;background:var(--bg);color:var(--fg);border:1px "
    "solid var(--bd);\n"
    ...
    " .actions "
    ".go{background:var(--acc);color:#06120d;border-color:var(--acc);font-"
    "weight:600}\n"
    " pre{background:#000;border:1px solid "
    "var(--bd);border-radius:5px;padding:10px;\n"
    "   "
    "white-space:pre-wrap;word-break:break-all;max-height:48vh;overflow:auto;"
    "margin:0}\n"
    " .err{color:var(--err)}\n"
    " .hint{color:var(--faint)}\n"
    " .rbar{display:flex;align-items:center;gap:10px;margin-bottom:8px}\n"
    " .rstatus{font-family:ui-monospace,monospace;font-size:12px;color:var(--"
    "mut)}\n"
    ...
    " table.rt th,table.rt td{border:1px solid var(--bd);padding:3px "
    "9px;text-align:left;vertical-align:top}\n"
    " table.rt thead "
    "th{background:var(--row);color:var(--mut);font-weight:600}\n"
    " table.rt tbody "
    "th{background:var(--lift);color:var(--faint);font-weight:500;font-family:"
    ...
    "<span class=\"up\" id=\"up\"></span><button "
    "id=\"reload\">reload</button></header>\n"
    "<main>\n"
    "  <div id=\"tree\">loading...</div>\n"
    "  <div id=\"panel\"><p class=\"hint\">pick a method on the "
    "left</p></div>\n"
    "</main>\n"
    "<script>\n"
    "const TOKEN=new URLSearchParams(location.search).get('token');\n"
    "function authH(h){h=h||{};if(TOKEN)h['Authorization']='Bearer "
    "'+TOKEN;return h;}\n"
    "const $=(t,c,x)=>{const "
    "e=document.createElement(t);if(c)e.className=c;if(x!=null)e.textContent=x;"
    "return e;};\n"
    "async function jget(u){const r=await fetch(u,{headers:authH({})});const "
    "t=await r.text();let "
    "d;try{d=JSON.parse(t);}catch(e){d=t;}return{ok:r.ok,status:r.status,data:"
    "d};}\n"
    "async function jrpc(path,args){const r=await "
    "fetch('/_rpc',{method:'POST',headers:authH({'Content-Type':'application/"
    "json'}),\n"
    "  body:JSON.stringify({path,args,kwargs:{}})});const t=await r.text();let "
    "d;try{d=JSON.parse(t);}catch(e){d=t;}return{ok:r.ok,status:r.status,data:"
    "d};}\n"
    "\n"
    ...
    "    if(v==='') return 0; const n=parseInt(v,10); return "
    "Number.isNaN(n)?v:n;}\n"
    "  if(t.includes('float')||t.includes('double')){\n"
    "    if(v==='') return 0; const n=parseFloat(v); return "
    "Number.isNaN(n)?v:n;}\n"
    "  return String(v);                              // char * and everything "
    "else\n"
    "}\n"
    "\n"
    ...
    "g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[c]))"
    ";}\n"
    "function isObj(v){return v!==null&&typeof "
    "v==='object'&&!Array.isArray(v);}\n"
    "function renderValue(v){\n"
    "  if(v===null||v===undefined) return '<span "
    "class=\"rt-null\">null</span>';\n"
    "  if(typeof v==='boolean') return v?'true':'false';\n"
    "  if(typeof v==='number') return esc(String(v));\n"
    "  if(typeof v==='string'){                              // a string that "
    "is itself JSON -> render recursively\n"
    "    const s=v.trim();\n"
    "    if(s.length>1&&(s[0]==='{'||s[0]==='[')){try{const "
    "p=JSON.parse(s);if(p&&typeof p==='object')return "
    "renderValue(p);}catch(e){}}\n"
    "    return esc(v);\n"
    "  }\n"
    "  if(Array.isArray(v)){\n"
    "    if(!v.length) return '<span class=\"rt-null\">[ empty ]</span>';\n"
    "    if(v.every(isObj)){                                  // list of "
    "records -> table\n"
    "      const keys=[...new Set(v.flatMap(o=>Object.keys(o)))];\n"
    "      let h='<table "
    "class=\"rt\"><thead><tr><th>#</th>'+keys.map(k=>'<th>'+esc(k)+'</"
    "th>').join('')+'</tr></thead><tbody>';\n"
    "      v.forEach((row,i)=>{h+='<tr><th>'+i+'</th>'+keys.map(k=>'<td>'+(k "
    "in row?renderValue(row[k]):'<span "
    "class=\"rt-null\">&mdash;</span>')+'</td>').join('')+'</tr>';});\n"
    "      return h+'</tbody></table>';\n"
    "    }\n"
    "    return '<ol "
    "class=\"rt-list\">'+v.map(x=>'<li>'+renderValue(x)+'</li>').join('')+'</"
    "ol>';\n"
    "  }\n"
    "  if(isObj(v)){\n"
    ...
    "    return '<table "
    "class=\"rt\"><tbody>'+e.map(([k,val])=>'<tr><th>'+esc(k)+'</"
    "th><td>'+renderValue(val)+'</td></tr>').join('')+'</tbody></table>';\n"
    "  }\n"
    "  return esc(String(v));\n"
    "}\n"
    ...
    "  out.innerHTML='';\n"
    "  const bar=$('div','rbar');\n"
    "  bar.appendChild($('span',res.ok?'rstatus':'rstatus err','HTTP "
    "'+res.status));\n"
    "  const tog=$('button','rtog','raw json');bar.appendChild(tog);\n"
    "  // unwrap {result: ...} / {error: ...}\n"
    "  let val=res.data;\n"
    "  if(val&&typeof val==='object'){ if('result'in val)val=val.result; else "
    "if('error'in val)val=val.error; }\n"
    "  const tbl=$('div','rbody');tbl.innerHTML=renderValue(val);\n"
    "  const raw=$('pre','rbody');raw.style.display='none';\n"
    "  raw.textContent=typeof "
    "res.data==='string'?res.data:JSON.stringify(res.data,null,2);\n"
    "  let showingRaw=false;\n"
    "  "
    "tog.onclick=()=>{showingRaw=!showingRaw;tbl.style.display=showingRaw?'"
    "none':'';raw.style.display=showingRaw?'':'none';tog.textContent="
    "showingRaw?'table':'raw json';};\n"
    ...
    "  if(!r.ok){tree.innerHTML='';tree.appendChild($('div','err','/_describe "
    "HTTP '+r.status));return;}\n"
    "  const svcs=(r.data&&r.data.services)||[];\n"
    "  tree.innerHTML='';\n"
    "  if(!svcs.length){tree.appendChild($('div','hint','no active "
    "services'));return;}\n"
    "  for(const s of svcs){\n"
    "    const box=$('div','svc closed');\n"
    "    const hd=$('div','hd');hd.appendChild($('span','tw','\\u25B6'));\n"
    "    "
    "hd.appendChild($('span',null,s.service));hd.appendChild($('span','badge',"
    "s.source||'?'));\n"
    "    "
    "hd.onclick=()=>{box.classList.toggle('closed');hd.querySelector('.tw')."
    "textContent=box.classList.contains('closed')?'\\u25B6':'\\u25BC';};\n"
    "    box.appendChild(hd);\n"
    "    const body=$('div','body');\n"
    "    for(const c of (s.classes||[])){\n"
    "      const cb=$('div','cls closed');\n"
    "      const "
    "clsName=c.class.indexOf(s.service+'.')===0?c.class.slice(s.service.length+"
    "1):c.class;\n"
    "      const "
    "ch=$('div','hd');ch.appendChild($('span','tw','\\u25B6'));ch.appendChild($"
    "('span',null,clsName));\n"
    "      "
    "ch.onclick=()=>{cb.classList.toggle('closed');ch.querySelector('.tw')."
    "textContent=cb.classList.contains('closed')?'\\u25B6':'\\u25BC';};\n"
    "      cb.appendChild(ch);\n"
    ...
    "        "
    "b.onclick=()=>{document.querySelectorAll('.m.sel').forEach(x=>x.classList."
    "remove('sel'));b.classList.add('sel');loadMethod(path);};\n"
    "        ms.appendChild(b);\n"
    "      }\n"
    ...
    "  }\n"
    "  // deep-link / demo: ?expand=1 opens the whole tree, "
    "?path=svc.cls.verb\n"
    "  // pre-selects a method (so a single screenshot shows the fields "
    "panel).\n"
    "  const q=new URLSearchParams(location.search);\n"
    "  const wp=q.get('path');\n"
    "  if(q.get('expand')==='1'||wp){\n"
    "    "
    "document.querySelectorAll('.svc,.cls').forEach(x=>x.classList.remove('"
    "closed'));\n"
    "    "
    ...
    "  if(wp){\n"
    "    const "
    "b=[...document.querySelectorAll('.m')].find(x=>x.dataset.path===wp);\n"
    ...
    "  if(!r.ok){panel.appendChild($('div','err','describe HTTP "
    "'+r.status));return;}\n"
    "  const params=(r.data&&r.data.params)||[];\n"
    "  panel.appendChild($('p','sub',params.length?(params.length+' "
    "parameter'+(params.length>1?'s':'')):'no parameters'));\n"
    "  const form=$('form');const inputs=[];\n"
    "  for(const p of params){\n"
    "    const f=$('div','field');\n"
    "    const lab=$('label');lab.appendChild($('span',null,p.name+' "
    "'));lab.appendChild($('span','ty',': '+p.type));\n"
    "    f.appendChild(lab);\n"
    "    const "
    "inp=$('input');inp.placeholder=p.type;inp.dataset.type=p.type;inp.dataset."
    "name=p.name;\n"
    "    f.appendChild(inp);form.appendChild(f);inputs.push(inp);\n"
    "  }\n"
    "  const fillRaw=q.get('args');                    // demo deep-link: "
    "prefill fields\n"
    "  if(fillRaw){try{const "
    "vals=JSON.parse(fillRaw);inputs.forEach((i,ix)=>{if(ix<vals.length)i."
    "value=String(vals[ix]);});}catch(e){}}\n"
    ...
    "  panel.appendChild($('div','hint','args sent positionally as "
    "['+params.map(p=>p.name).join(', ')+']'));\n"
    "  panel.appendChild(out);\n"
    "  if(q.get('invoke')==='1')go.click();            // demo deep-link: "
    "auto-run\n"
    "}\n"
    "\n"
...
static void send_resp(struct loop_stream *stream, int status,
  ...
  if (blen)
    loop_write(stream, body, blen);
}

static int hdr_match(const struct phr_header *hdrs, size_t count,
                     const char *want, char *out, size_t out_cap) {
  size_t want_len = strlen(want);
  for (size_t i = 0; i < count; ++i) {
    if (hdrs[i].name_len != want_len)
      continue;
    if (strncasecmp(hdrs[i].name, want, want_len) != 0)
      continue;
    size_t copy_len =
...
static int query_get(const char *path, const char *key, char *out,
                     size_t out_cap) {
  const char *query = strchr(path, '?');
  if (!query)
    return 0;
  ...
  while (*scan) {
    ...
    if (equals && (!ampersand || equals < ampersand) &&
        (size_t)(equals - scan) == key_len && memcmp(scan, key, key_len) == 0) {
...
static int wants_proxy(const char *path, int is_get, int is_post) {
  if (is_post && path_eq(path, "/_rpc"))
    return 1;
  if (is_get || is_post) {
    if (path_eq(path, "/_describe") || path_eq(path, "/_describe_tree"))
      return 1;
    if (path_ends(path, "/_describe") || path_ends(path, "/_describe_tree"))
      return 1;
  }
  return 0;
}

static int authorized(const struct alpine_frontend *frontend,
                      const struct phr_header *hdrs, size_t header_count,
                      const char *path) {
  if (!frontend->token)
    return 1; /* no token configured: open (loopback admin tool) */
  char got[300] = {0};
  char auth[300] = {0};
  ...
  if (!got[0])
    query_get(path, "token", got, sizeof(got));
...
proxy_upstream(struct loop *loop, struct loop_stream *client,
               const struct alpine_frontend *frontend, const char *method,
               const char *path, const char *ctype, const char *body,
               size_t blen) {
  struct loop_stream_ptr_result upstream_res =
      loop_connect_tcp(loop, frontend->up_host, frontend->up_port);
  if (PICOMESH_IS_ERR(upstream_res)) {
    picomesh_error_print(stderr, "alpine proxy: upstream connect",
                         upstream_res.error);
    picomesh_error_destroy(upstream_res.error);
    static const char msg[] = "{\"error\":\"alpine: upstream connect failed\"}";
    send_resp(client, 502, "Bad Gateway", "application/json", msg,
              sizeof(msg) - 1);
  ...
  if (head_len <= 0 || (size_t)head_len >= sizeof(head)) {
    ...
    static const char msg[] =
        "{\"error\":\"alpine: request too large to proxy\"}";
    send_resp(client, 502, "Bad Gateway", "application/json", msg,
              sizeof(msg) - 1);
    return PICOMESH_OK_VOID();
  }
  ...
  for (;;) {
    if (response_len >= ALPINE_RESP_BUF)
      break;
    struct picomesh_size_result read_res = loop_read_some(
        upstream, response + response_len, ALPINE_RESP_BUF - response_len);
  ...
  loop_close(upstream);
  if (response_len)
    loop_write(client, response, response_len);
  free(response);
  return PICOMESH_OK_VOID();
}
...
static void serve_one(struct loop *loop, struct loop_stream *stream, void *ud) {
  ...
  while (parsed == -2) {
    ...
    parsed = phr_parse_request(buf, buf_len, &method_ptr, &method_len, &path,
                               &path_len, &minor, hdrs, &header_count, 0);
  }
  if (parsed < 0)
    goto done;
  ...
  if (hdr_match(hdrs, header_count, "Content-Length", content_len_str,
                sizeof(content_len_str)))
    clen = strtol(content_len_str, NULL, 10);
  if (clen > 0) {
    size_t have = buf_len - header_end;
    while ((long)have < clen) {
      if (header_end + (size_t)clen > ALPINE_REQ_BUF)
        goto done;
      struct picomesh_size_result read_res =
          loop_read_some(stream, buf + buf_len, (size_t)clen - have);
      if (PICOMESH_IS_ERR(read_res)) {
        picomesh_error_destroy(read_res.error);
        goto done;
      }
  ...
  ((char *)path)[path_len] =
      0; /* NUL-terminate path (byte after is part of HTTP/1.1) */

  char method[16] = {0};
  memcpy(method, method_ptr,
         method_len < sizeof(method) - 1 ? method_len : sizeof(method) - 1);
  int is_get = strcmp(method, "GET") == 0;
  int is_post = strcmp(method, "POST") == 0;
  int is_opt = strcmp(method, "OPTIONS") == 0;

  if (!authorized(frontend, hdrs, header_count, path)) {
    static const char msg[] = "{\"error\":\"unauthorized: pass ?token= or "
                              "Authorization: Bearer <token>\"}";
    send_resp(stream, 401, "Unauthorized", "application/json", msg,
              sizeof(msg) - 1);
    goto done;
  }
  ...
  if (wants_proxy(path, is_get, is_post)) {
    ...
    struct picomesh_void_result proxy_res = proxy_upstream(
        loop, stream, frontend, method, path,
        ctype[0] ? ctype : "application/json", body, (size_t)clen);
...

  {
    static const char not_found[] =
        "{\"error\":\"alpine: no such route (serves /_alpine; proxies "
        "/_describe and /_rpc)\"}";
    send_resp(stream, 404, "Not Found", "application/json", not_found,
              sizeof(not_found) - 1);
  }
...
static const struct config_node *alpine_cfg(struct picomesh_engine *engine,
                                            const char *suffix) {
  ...
  char path[256];
  if (name && *name) {
    snprintf(path, sizeof(path), "mesh.services.%s.config.alpine.%s", name,
             suffix);
...
static int alpine_upstream_cb(const char *key, const struct config_node *val,
                              void *ud) {
  struct alpine_upstream_fields *upstream = ud;
  if (strcmp(key, "host") == 0) {
    const char *host = config_node_as_string(val, NULL);
    if (host)
      snprintf(upstream->host, sizeof(upstream->host), "%s", host);
...
struct alpine_frontend_ptr_result
alpine_start(struct picomesh_engine *engine, const struct alpine_config *cfg) {
  if (!engine)
    return PICOMESH_ERR(alpine_frontend_ptr, "alpine_start: NULL engine");
  ...
  if (!loop)
    return PICOMESH_ERR(alpine_frontend_ptr,
                        "alpine_start: engine has no loop");
  ...
  if (!frontend)
    return PICOMESH_ERR(alpine_frontend_ptr, "alpine_start: calloc failed");
  frontend->engine = engine;
  snprintf(frontend->up_host, sizeof(frontend->up_host), "%s",
           upstream.host[0] ? upstream.host : "127.0.0.1");
  frontend->up_port = upstream.port;

  const struct config_node *token_node = alpine_cfg(engine, "token");
  const char *token =
      token_node ? config_node_as_string(token_node, NULL) : NULL;
  if (token && *token) {
    frontend->token = strdup(token);
    if (!frontend->token) {
      free(frontend);
      return PICOMESH_ERR(alpine_frontend_ptr, "alpine_start: strdup failed");
    }
  }
  ...
  if (PICOMESH_IS_ERR(listen_res)) {
    ...
    free(frontend);
    return PICOMESH_ERR(alpine_frontend_ptr,
                        "alpine_start: loop_listen_tcp failed", listen_res);
  }
  yinfo("alpine: console on %s:%d -> upstream %s:%d (%s)", host, port,
        frontend->up_host, frontend->up_port,
        frontend->token ? "token-gated" : "open");
