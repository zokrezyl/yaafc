                                       ...
                                       const struct class *const *mixins, size_t mixin_count)
{
    ...
    /* Registering method slots mutates the process-global slot table, and if a
     * later step (slice grow / registry add) fails we class_destroy(cls) without
     * unregistering these slots. That is INTENTIONAL and safe — do NOT add slot
     * rollback here:
     *   - method_slot_register is idempotent: a slot is keyed by (domain, name),
     *     so a retry of this same class re-resolves the identical slots, and two
     *     classes in one domain may legitimately share a slot name (unregistering
     *     would corrupt the other class's dispatch).
     *   - slots are program-lifetime: classes register once at startup; a failure
     *     here is fatal to bootstrap, there is no live "retry against a poisoned
     *     table" path in practice.
     *   - a leftover slot whose class never made it into the registry has no impl
     *     in any dispatch slice, so a lookup of it fails cleanly (no impl) rather
...
struct picomesh_void_result class_for_each_slot(const struct class *cls,
                         void (*cb)(const char *name, method_slot slot, void *ud), void *userdata)
{
    ydebug("cls=%s", cls && cls->desc ? cls->desc->name : "(null)");
    if (!cls || !cb) return PICOMESH_OK_VOID();
    for (uint8_t domain_id = 0; domain_id < METHOD_SLOT_MAX_DOMAINS; ++domain_id) {
        const struct dispatch_slice *slice = &cls->dispatch_by_domain[domain_id];
        for (size_t i = 0; i < slice->count; ++i) {
            ...
            PICOMESH_RETURN_IF_ERR(picomesh_void, name_res, "class_for_each_slot: method slot name lookup failed");
            if (name_res.value) cb(name_res.value, slot, userdata);
        }
    }
    return PICOMESH_OK_VOID();
}
