...
def emit_public_header(model: dict, module: str, out_path: Path):
    ...
    parts.append("\n/* ---- activation ---- */\n")
    parts.append(f"void picomesh_plugin_{module}_register(void);\n")
...
"""
    ...
    prewarm_calls = "\n".join(
        ...
        f"      if (PICOMESH_IS_ERR(reg)) picomesh_error_destroy(reg.error); }}"
        for c in classes
    )
...
void picomesh_plugin_{module}_register(void)
{{
    struct picomesh_void_result _ar = class_add_accessor_lookup({module}_accessor_lookup);
    if (PICOMESH_IS_ERR(_ar)) {{
        picomesh_error_print(stderr, "picomesh_plugin_{module}_register", _ar.error);
...
def main():
    ...
    placeholder = '#include <picomesh/yclass/class.h>\n'
