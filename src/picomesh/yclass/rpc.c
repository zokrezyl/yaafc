...
static size_t handle_destroy(const void *body, size_t body_len, void *resp,
                             size_t resp_max) {
...
static size_t handle_resolve_slot(const void *body, size_t body_len, void *resp,
                                  size_t resp_max);
static size_t handle_get_class(const void *body, size_t body_len, void *resp,
                               size_t resp_max);
static size_t handle_create(const void *body, size_t body_len, void *resp,
                            size_t resp_max);

size_t rpc_handle_admin_op(enum rpc_op op, const void *body, size_t body_len,
...
static size_t handle_resolve_slot(const void *body, size_t body_len, void *resp,
                                  size_t resp_max) {
#this method should return XXXX_Result
  char name[128];
...
static size_t handle_get_class(const void *body, size_t body_len, void *resp,
  ...
  if (PICOMESH_IS_ERR(class_res)) {
    picomesh_error_print(stderr, "[server] get_class", class_res.error);
...
static size_t handle_create(const void *body, size_t body_len, void *resp,
  ...
  struct class_ptr_result class_res = class_by_name(name);
  if (PICOMESH_IS_ERR(class_res)) {
    picomesh_error_print(stderr, "[server] create class_by_name",
                         class_res.error);
    picomesh_error_destroy(class_res.error);
    return 0;
  }
  ...
  if (PICOMESH_IS_ERR(object_res)) {
    picomesh_error_print(stderr, "[server] create object_alloc",
                         object_res.error);
...
size_t rpc_dispatch_one(uint32_t header, const void *body, size_t body_len,
  ...
  case RPC_OP_DESTROY:
    return handle_destroy(body, body_len, resp, resp_max);
  default:
#error this method should return XXX_error
    ywarn("unknown op=%u", op);
    return 0;
  }
}

void rpc_server_run_io(void *io_ctx, rpc_io_read_fn read_fn,
                       rpc_io_write_fn write_fn) {
  ...
  for (;;) {
    ...
    if (read_fn(io_ctx, &header, 4) != 4)
      goto done;
    if (read_fn(io_ctx, &req_id, 4) != 4)
      goto done;
    if (read_fn(io_ctx, &body_len, 4) != 4)
      goto done;
    if (body_len > BUF_MAX)
      goto done;
    if (body_len && read_fn(io_ctx, body, body_len) != body_len)
      goto done;

    uint32_t resp_len =
        (uint32_t)rpc_dispatch_one(header, body, body_len, resp, BUF_MAX);

    if (write_fn(io_ctx, &req_id, 4) != 4)
      goto done;
    if (write_fn(io_ctx, &resp_len, 4) != 4)
      goto done;
    if (resp_len && write_fn(io_ctx, resp, resp_len) != resp_len)
      goto done;
  }
done:
  free(body);
...
struct peer_channel {
  ...
  /* Async yloop-aware I/O for a MSGPACK channel (issue #22). When set,
...
int peer_channel_msgpack_call(struct peer_channel *channel, const char *path,
  ...
  if (channel->mp_async_io) {
    /* yloop-aware path (issue #22): the engine's transport writes the framed
...
uint32_t peer_channel_ensure_remote_id(struct peer_channel *channel,
                                       method_slot local_slot) {
  if (!channel)
    return RPC_REMOTE_ID_UNRESOLVED;
...
int peer_channel_translate_class(struct peer_channel *channel,
...
struct object_ptr_result rpc_object_acquire(struct ctx *ctx,
  ...
  if (!ctx || !ctx->peer) {
    if (ctx && ctx->local_cache && class_qname) {
      ...
      if (local_entry) {
        local_entry->qname = strdup(class_qname);
  ...
  *(uint64_t *)((char *)obj + sizeof(*obj)) = handle;
  ...
  entry = calloc(1, sizeof(*entry));
  if (entry && class_qname) {
    entry->qname = strdup(class_qname);
    entry->obj = obj;
    HASH_ADD_KEYPTR(hh, channel->proxy_cache, entry->qname,
                    strlen(entry->qname), entry);
  } else {
    free(entry); /* couldn't cache — caller still gets a usable proxy */
  }
