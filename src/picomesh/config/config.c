...
static int map_set(struct config_node *map, const char *key,
                   struct config_node *value) {
  /* Replace if exists. */
  ...
  if (map->u.map.count == map->u.map.cap) {
    ...
    struct kv *new_entries =
        realloc(map->u.map.entries, new_cap * sizeof(*new_entries));
    if (!new_entries)
      return -1;
  ...
  if (!map->u.map.entries[map->u.map.count].key)
    return -1;
...
static struct config_node *map_get(const struct config_node *map,
                                   const char *key) {
  if (!map || map->kind != CONFIG_MAP)
    return NULL;
  for (size_t i = 0; i < map->u.map.count; ++i) {
    if (strcmp(map->u.map.entries[i].key, key) == 0) {
...
static int list_push(struct config_node *list, struct config_node *item) {
  if (list->u.list.count == list->u.list.cap) {
    ...
    struct config_node **new_items =
        realloc(list->u.list.items, new_cap * sizeof(*new_items));
    if (!new_items)
      return -1;
...
static struct config_node *node_from_scalar(const yaml_event_t *ev) {
  ...
  if (!quoted) {
    ...
    if ((len == 4 &&
        ...
        (len == 3 &&
         (strncmp(val, "yes", 3) == 0 || strncmp(val, "Yes", 3) == 0)) ||
        (len == 2 &&
    ...
    if ((len == 5 &&
         (strncmp(val, "false", 5) == 0 || strncmp(val, "False", 5) == 0)) ||
        (len == 2 &&
         (strncmp(val, "no", 2) == 0 || strncmp(val, "No", 2) == 0))) {
      struct config_node *node = node_new(CONFIG_BOOL);
      if (node)
        node->u.b = 0;
      return node;
...
static int attach(struct frame *top, struct config_node *child) {
  ...
  if (!top->pending_key) {
    if (child->kind != CONFIG_STRING) {
      ...
      switch (child->kind) {
      case CONFIG_INT:
        snprintf(buf, sizeof(buf), "%" PRId64, child->u.i);
        break;
      case CONFIG_FLOAT:
        snprintf(buf, sizeof(buf), "%g", child->u.f);
        break;
      case CONFIG_BOOL:
        snprintf(buf, sizeof(buf), "%s", child->u.b ? "true" : "false");
        break;
      default:
...
static struct config_node *parse_stream(yaml_parser_t *parser) {
  ...
  for (;;) {
    ...
    case YAML_MAPPING_START_EVENT: {
      ...
      if (!root)
        root = map;
      else if (attach(&stack[depth - 1], map) < 0)
        goto err;
      if (push_frame(&stack, &depth, &cap, map) < 0)
        goto err;
      break;
    }
    case YAML_SEQUENCE_START_EVENT: {
      ...
      if (!root)
        root = list;
      else if (attach(&stack[depth - 1], list) < 0)
        goto err;
      if (push_frame(&stack, &depth, &cap, list) < 0)
        goto err;
      break;
    }
...
static struct config_node *parse_yaml_file(const char *path) {
  ...
  if (!root) {
    ...
    ywarn("config: %s: top-level is not a map (got kind=%d), ignoring", path,
          root->kind);
    node_free(root);
    root = node_new(CONFIG_MAP);
  }
...
static int merge_into(struct config_node *base,
                      const struct config_node *over) {
  if (!base || !over || base->kind != CONFIG_MAP || over->kind != CONFIG_MAP)
    return -1;
  for (size_t i = 0; i < over->u.map.count; ++i) {
    ...
    if (existing && existing->kind == CONFIG_MAP && value->kind == CONFIG_MAP) {
      if (merge_into(existing, value) < 0)
        return -1;
    } else {
  ...
  for (const char *cursor = src; *cursor;) {
    if (cursor[0] == '$' && cursor[1] == '{') {
      ...
      if (env_value) {
        ...
        if (default_len >= sizeof(def_buf))
          default_len = sizeof(def_buf) - 1;
...
static void subst_env(struct config_node *node) {
  ...
  case CONFIG_LIST:
    for (size_t i = 0; i < node->u.list.count; ++i)
      subst_env(node->u.list.items[i]);
    break;
  case CONFIG_MAP:
    for (size_t i = 0; i < node->u.map.count; ++i)
      subst_env(node->u.map.entries[i].value);
    break;
  default:
    break;
  }
...
static int apply_cli_override(struct config_node *root, const char *kv) {
  ...
  if (rest_len >= sizeof(buf)) {
    node_free(leaf);
    return 0;
  }
  memcpy(buf, rest, rest_len + 1);
  for (char *tok = strtok(buf, "."); tok; tok = strtok(NULL, ".")) {
    if (tcount >= sizeof(tokens) / sizeof(tokens[0]))
      break;
    tokens[tcount++] = tok;
  }
...
struct config_ptr_result config_create(const struct config_create_args *args) {
  ...
  if (!config)
    return PICOMESH_ERR(config_ptr, "config_create: calloc failed");
   ...
   * a second mesh started with `--config
   * mesh.nodes_dir=/tmp/picoforge2/nodes`). yaapp applied these as defaults
   * instead, which cannot override a file. */
  ...
  if (args && args->cli_overrides) {
    for (size_t i = 0; i < args->cli_override_count; ++i) {
      if (apply_cli_override(config->root, args->cli_overrides[i]) < 0) {
        ...
        return PICOMESH_ERR(config_ptr,
                            "config_create: cli override merge failed");
      }
    }
  }
...
struct picomesh_void_result config_promote_subtree(struct config *config,
                                                   const char *dot_path) {
  if (!config || !dot_path) {
...
static const struct config_node *walk_path(const struct config_node *node,
                                           const char *path) {
  if (!node || !path)
    return NULL;
  char buf[256];
  ...
  for (char *tok = strtok(buf, "."); tok; tok = strtok(NULL, ".")) {
    if (!current || current->kind != CONFIG_MAP)
      return NULL;
 ...
 */
static const struct config_node *resolve_path(const struct config *config,
                                              const char *dot_path) {
  if (!config || !dot_path)
    return NULL;
...
struct config_node_ptr_result config_get(const struct config *config,
                                         const char *dot_path) {
  if (!config || !dot_path) {
...
const struct config_node *config_get_node(const struct config *config,
                                          const char *dot_path) {
  return resolve_path(config, dot_path);
}

struct config_node_ptr_result config_require(const struct config *config,
                                             const char *dot_path) {
  if (!config || !dot_path)
  ...
  if (!node) {
    char msg[300];
    snprintf(msg, sizeof(msg), "config: required key '%s' is missing",
             dot_path);
 ...
 */
const char *config_get_string(const struct config *config, const char *dot_path,
                              const char *fallback) {
  return config_node_as_string(resolve_path(config, dot_path), fallback);
}

int64_t config_get_int(const struct config *config, const char *dot_path,
                       int64_t fallback) {
  return config_node_as_int(resolve_path(config, dot_path), fallback);
}

int config_get_bool(const struct config *config, const char *dot_path,
                    int fallback) {
  return config_node_as_bool(resolve_path(config, dot_path), fallback);
}

const struct config_node *config_section(const struct config *config,
                                         const char *name) {
...
size_t config_node_size(const struct config_node *node) {
  if (!node)
    return 0;
  if (node->kind == CONFIG_MAP)
    return node->u.map.count;
  if (node->kind == CONFIG_LIST)
    return node->u.list.count;
  return 0;
}

const char *config_node_as_string(const struct config_node *node,
                                  const char *fallback) {
...
int64_t config_node_as_int(const struct config_node *node, int64_t fallback) {
  if (!node)
    return fallback;
  if (node->kind == CONFIG_INT)
    return node->u.i;
  if (node->kind == CONFIG_FLOAT)
    return (int64_t)node->u.f;
  if (node->kind == CONFIG_BOOL)
    return (int64_t)node->u.b;
  if (node->kind == CONFIG_STRING) {
    int64_t value;
    if (parse_int64(node->u.s, &value) == 0)
      return value;
  }
  return fallback;
}
...
double config_node_as_float(const struct config_node *node, double fallback) {
  ...
  if (node->kind == CONFIG_FLOAT)
    return node->u.f;
  if (node->kind == CONFIG_INT)
    return (double)node->u.i;
  if (node->kind == CONFIG_STRING) {
    double value;
    if (parse_float(node->u.s, &value) == 0)
      return value;
  }
  return fallback;
}
...
int config_node_for_each(const struct config_node *node,
                         ...
                         void *ud) {
  if (!node || node->kind != CONFIG_MAP || !cb)
    return 0;
  for (size_t i = 0; i < node->u.map.count; ++i) {
    int result_code =
        cb(node->u.map.entries[i].key, node->u.map.entries[i].value, ud);
    if (result_code)
      return result_code;
  }
  return 0;
...
const struct config_node *config_node_at(const struct config_node *node,
                                         size_t idx) {
  if (!node || node->kind != CONFIG_LIST || idx >= node->u.list.count)
    return NULL;
  return node->u.list.items[idx];
}

const struct config_node *config_node_get(const struct config_node *node,
                                          const char *key) {
  if (!node || node->kind != CONFIG_MAP || !key)
    return NULL;
  for (size_t i = 0; i < node->u.map.count; ++i)
    if (strcmp(node->u.map.entries[i].key, key) == 0)
      return node->u.map.entries[i].value;
  return NULL;
}

/* ---------------- pretty-print ------------------------------------ */

static int dump_node(const struct config_node *n, char *buf, size_t cap,
                     size_t off, int indent);
...
static int dump_node(const struct config_node *node, char *buf, size_t cap,
                     size_t off, int indent) {
  if (!node)
    return (int)off;
  char tmp[64];
  switch (node->kind) {
  case CONFIG_NULL:
    off = (size_t)append(buf, cap, off, "null");
    break;
  case CONFIG_BOOL:
    off = (size_t)append(buf, cap, off, node->u.b ? "true" : "false");
    break;
  case CONFIG_INT:
    snprintf(tmp, sizeof(tmp), "%" PRId64, node->u.i);
    off = (size_t)append(buf, cap, off, tmp);
    break;
  case CONFIG_FLOAT:
    snprintf(tmp, sizeof(tmp), "%g", node->u.f);
    off = (size_t)append(buf, cap, off, tmp);
    break;
  case CONFIG_STRING:
  ...
  case CONFIG_MAP:
    ...
    for (size_t i = 0; i < node->u.map.count; ++i) {
      ...
      off = (size_t)append(buf, cap, off, ": ");
      off = (size_t)dump_node(node->u.map.entries[i].value, buf, cap, off,
                              indent + 1);
      off = (size_t)append(buf, cap, off, ",\n");
    }
...
size_t config_dump(const struct config *config, char *buf, size_t bufsize) {
  ...
  if ((size_t)dumped_len >= bufsize)
    dumped_len = (int)bufsize - 1;
  buf[dumped_len] = 0;
  return (size_t)dumped_len;
}

size_t config_node_dump(const struct config_node *node, char *buf,
                        size_t bufsize) {
  if (!buf || bufsize == 0)
    return 0;
  int written_len = dump_node(node, buf, bufsize, 0, 0);
  if ((size_t)written_len >= bufsize)
    written_len = (int)bufsize - 1;
  buf[written_len] = 0;
