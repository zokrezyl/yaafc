...
static void apply_cli_env(const struct yargv_chain *cli)
{
    ...
    if (!cli) return;
    const char *envs[64];
    size_t count = yargv_get_kv_list(cli, "env", envs, sizeof(envs) / sizeof(envs[0]));
...
struct picomesh_engine_ptr_result picomesh_engine_create(const struct picomesh_engine_args *args)
{
    ...
    if (!config_file && engine->cli) {
        config_file = yargv_get_string(engine->cli, "config_file", NULL);
    }

    const char *cli_overrides[64];
    size_t cli_override_count = 0;
    if (engine->cli) {
        cli_override_count = yargv_get_kv_list(engine->cli, "config", cli_overrides,
    ...
    struct yconfig_ptr_result config_res = yconfig_create(&cfg_args);
    if (PICOMESH_IS_ERR(config_res)) {
        free(engine);
        return PICOMESH_ERR(picomesh_engine_ptr, "picomesh_engine_create: yconfig_create failed", config_res);
    }
    ...
    if (engine->cli) self_name = yargv_get_string(engine->cli, "name", NULL);
    if (self_name && *self_name) {
        char path[256];
        snprintf(path, sizeof(path), "mesh.services.%s.config", self_name);
        struct picomesh_void_result promote_res = yconfig_promote_subtree(engine->config, path);
    ...
    if (PICOMESH_IS_ERR(loop_res)) {
        ...
        yconfig_destroy(engine->config);
        free(engine);
        return PICOMESH_ERR(picomesh_engine_ptr, "picomesh_engine_create: yloop_create failed", loop_res);
    }
...
void picomesh_engine_destroy(struct picomesh_engine *engine)
{
    ...
    for (size_t i = 0; i < engine->worker_count; ++i) {
        ...
        if (engine->workers[i].flush_timer) yloop_timer_stop(engine->workers[i].flush_timer);
        yloop_destroy(engine->workers[i].loop);
    }
    free(engine->workers);
    yconfig_destroy(engine->config);
    yargv_chain_destroy(engine->cli);
    free(engine);
}
...
static int rpc_async_drain(struct yloop_stream *stream, uint32_t remaining)
{
    uint8_t scratch[512];
    while (remaining) {
...
static void rpc_async_reader_entry(void *arg)
{
    ...
    struct yloop_stream *stream = client->stream;
    ...
    for (;;) {
        ...
        if (resp_len && yloop_read(stream, call->resp, resp_len) != resp_len) break;
...
static int rpc_async_ensure_connected(struct rpc_async_client *client)
{
    ...
    int ok = 0;
    struct yloop_stream_ptr_result stream_res = yloop_connect_tcp(client->loop, client->host, client->port);
                                    ...
                                    void *resp, size_t resp_max)
{
    ...
    if (!rpc_async_ensure_connected(client)) return 0;
    ...
    size_t wrote = yloop_write(client->stream, frame, frame_len);
    free(frame);
    if (wrote != frame_len) {
...
{
    struct rpc_async_client *client = vclient;
    if (!rpc_async_ensure_connected(client)) return;
    ...
    memcpy(frame + 8, &blen, 4);
    if (body_len) memcpy(frame + 12, body, body_len);
    yloop_write(client->stream, frame, frame_len); /* yields; result ignored */
    free(frame);
}
...
static void rpc_async_client_destroy(void *vclient)
{
    ...
    if (client->stream) yloop_close(client->stream);
    free(client->host);
    free(client);
}
 ...
 *
 * The yloop-aware twin of open_async_session, but speaking the Picomesh msgpack
...
static int msgpack_async_roundtrip(struct msgpack_async_client *client, const uint8_t *env,
                                   size_t env_len, uint8_t **resp_out, size_t *resp_len_out,
                                   char *err, size_t err_cap)
{
    if (!client->stream) {
        struct yloop_stream_ptr_result connect_res =
            yloop_connect_tcp(client->loop, client->host, client->port);
    ...
    if (yloop_write(client->stream, lenbuf, 4) != 4 ||
        yloop_write(client->stream, env, env_len) != env_len) {
    ...
    if (yloop_read(client->stream, resp_lenbuf, 4) != 4) {
    ...
    if (yloop_read(client->stream, resp, resp_len) != resp_len) {
        free(resp);
                            ...
                            size_t *resp_len_out, char *err, size_t err_cap)
{
    ...
    *resp_len_out = 0;
    ...
    int result = msgpack_async_roundtrip(client, env, env_len, resp_out, resp_len_out, err, err_cap);
...
static struct peer_channel *open_async_msgpack_session(struct yloop *loop, const char *host,
                                                       int port)
{
...
static struct peer_channel *open_remote_session(struct yloop *loop, const char *host, int port,
 ...
 * success. */
static int resolve_service_bind(struct picomesh_engine *engine, const char *service,
                                char *host_out, size_t host_cap, int *port_out)
{
    char path[256];
    ...
    if (PICOMESH_IS_OK(host_res) && host_res.value) {
        host = yconfig_node_as_string(host_res.value, NULL);
    }
    ...
    if (PICOMESH_IS_OK(port_res) && port_res.value) {
        port = yconfig_node_as_int(port_res.value, -1);
    }
    if (port <= 0) return 0;
...
int picomesh_engine_service_addr(struct picomesh_engine *engine, const char *service,
                              char *host_out, size_t host_cap, int *port_out)
{
    if (!engine || !service || !host_out || !port_out) return 0;
                                ...
                                void *ud)
{
    ...
    if (strcmp(key, "service") == 0) {
        fields->svc = yconfig_node_as_string(val, NULL);
    } else if (strcmp(key, "host") == 0) {
        fields->host = yconfig_node_as_string(val, NULL);
    } else if (strcmp(key, "port") == 0) {
        fields->port = (int)yconfig_node_as_int(val, 0);
    } else if (strcmp(key, "transport") == 0) {
        fields->transport = yconfig_node_as_string(val, NULL);
    }
    return 0;
}

size_t picomesh_engine_open_remotes(struct picomesh_engine *engine, const char *plugin)
{
    if (!engine || !plugin) return 0;

    char path[256];
    snprintf(path, sizeof(path), "mesh.services.%s.config.remotes", plugin);
    struct yconfig_node_ptr_result list_res =
        yconfig_get(picomesh_engine_config(engine), path);
    if (PICOMESH_IS_ERR(list_res) || !list_res.value) return 0;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        yconfig_node_for_each(entry, remote_entry_walk_cb, &fields);
        ...
        if (port <= 0) {
            char bind_host[128];
            int bind_port = 0;
            if (resolve_service_bind(engine, fields.svc, bind_host, sizeof(bind_host), &bind_port)) {
...
struct picomesh_void_result picomesh_engine_run(struct picomesh_engine *engine)
{
    ...
    *current_worker_slot() = &engine->workers[0];
    worker_start_flush_timer(&engine->workers[0]);
    struct picomesh_void_result run_res = yloop_run(engine->workers[0].loop);
    PICOMESH_RETURN_IF_ERR(picomesh_void, run_res, "picomesh_engine_run: yloop_run failed");
    return PICOMESH_OK_VOID();
}
...
static void worker_start_flush_timer(struct picomesh_worker *worker)
{
    ...
    if (PICOMESH_IS_ERR(spawn_res)) {
        ...
        picomesh_error_print(stderr, "worker_start_flush_timer: spawn ytel-flush", spawn_res.error);
        picomesh_error_destroy(spawn_res.error);
        return;
    }
    picomesh_coro_resume(spawn_res.value); /* runs to its first yloop_sleep_ms yield */
}
...
static void *worker_thread_main(void *arg)
{
    ...
    *current_worker_slot() = worker;
    ...
    worker_start_flush_timer(worker);
    struct picomesh_void_result run_res = yloop_run(worker->loop);
    if (PICOMESH_IS_ERR(run_res)) {
        ...
        picomesh_error_print(stderr, "worker_thread_main: yloop_run", run_res.error);
                                                  ...
                                                  void *ud)
{
    ...
    *current_worker_slot() = &engine->workers[0];
    ...
    for (size_t i = 1; i < engine->worker_count; ++i) {
        if (engine->workers[i].started && engine->workers[i].loop) yloop_stop(engine->workers[i].loop);
    }
    for (size_t i = 1; i < engine->worker_count; ++i) {
...
void picomesh_engine_stop(struct picomesh_engine *engine)
{
    if (!engine) return;
    for (size_t i = 0; i < engine->worker_count; ++i) {
        if (engine->workers[i].loop) yloop_stop(engine->workers[i].loop);
    }
}
...
struct picomesh_void_result picomesh_engine_perf_start(struct picomesh_engine *engine, const char *service)
{
     ...
     * level directly). */
    const struct yconfig_node *perf_node = yconfig_section(engine->config, "perf");
...
struct yloop *picomesh_engine_loop(struct picomesh_engine *engine)
{
    if (!engine) return NULL;
    return engine_current_worker(engine)->loop;
}

const struct yconfig *picomesh_engine_config(struct picomesh_engine *engine)
{
    return engine ? engine->config : NULL;
}

const struct yconfig_node *picomesh_engine_plugin_config(struct picomesh_engine *engine, const char *plugin)
{
    return yconfig_section(engine ? engine->config : NULL, plugin);
}

struct yargv_chain *picomesh_engine_cli(struct picomesh_engine *engine)
{
